...
SausagePlacePlan makeSausagePlacePlan(
  ...
  const double release_z = held_case_center.z() + case_target.size.z() * 0.5 +
                           bread_target.size.z() + sausage_thickness * 0.5 +
  ...
  const auto* work_preset = task_presets::findStageWaypointPosePreset(
      task_presets::ManufacturingTarget::Sausage, task_presets::ArmSide::Left,
      task_presets::ManufacturingStage::Work, "work");
  const auto* place_preset = task_presets::findStageWaypointPosePreset(
      task_presets::ManufacturingTarget::Sausage, task_presets::ArmSide::Left,
      task_presets::ManufacturingStage::Place, "release");
  ...
  if (work_preset != nullptr) {
    if (has_completed_left_work_pose) {
      ...
      plan.approach_pose =
          makePoseFromWaypointPreset(logger, *work_preset, references, "Sausage place approach");
      RCLCPP_INFO(logger, "Sausage place approach pose preset applied");
    }
  } else {
    RCLCPP_INFO(logger, "Sausage place auto approach pose: xyz=[%.3f %.3f %.3f]",
                plan.approach_pose.position.x, plan.approach_pose.position.y,
  ...
  if (place_preset != nullptr) {
    ...
    RCLCPP_INFO(
        ...
        plan.release_pose.position.x, plan.release_pose.position.y, plan.release_pose.position.z,
        task_presets::kLeftSausageReleaseYOffsetFromApproachM);
  }

  if (has_completed_left_work_pose && place_preset == nullptr) {
    RCLCPP_INFO(logger,
                "Sausage place release keeps completed work xy and lowers vertically to computed "
                "release height");
  }
...
KetchupSqueezePlan makeKetchupSqueezePlan(
  ...
  const double sausage_thickness = topDownPlaceThickness(sausage_target);
  const double squeeze_z = held_case_center.z() + case_target.size.z() * 0.5 +
                           bread_target.size.z() + sausage_thickness + ketchup_squeeze_height_m;
  ...
  const auto* work_stage_preset = task_presets::findStageWaypointPosePreset(
      task_presets::ManufacturingTarget::Ketchup, task_presets::ArmSide::Left,
      task_presets::ManufacturingStage::Work, "work");
  const auto* aim_preset = task_presets::findStageWaypointPosePreset(
      task_presets::ManufacturingTarget::Ketchup, task_presets::ArmSide::Left,
      task_presets::ManufacturingStage::Work, "aim");
  const auto* squeeze_preset = task_presets::findStageWaypointPosePreset(
      task_presets::ManufacturingTarget::Ketchup, task_presets::ArmSide::Left,
      task_presets::ManufacturingStage::Work, "squeeze");
  ...
  if (aim_preset != nullptr) {
    ...
    RCLCPP_INFO(logger, "Ketchup auto aim pose: xyz=[%.3f %.3f %.3f]", plan.aim_pose.position.x,
                plan.aim_pose.position.y, plan.aim_pose.position.z);
  }
  ...
  if (squeeze_preset != nullptr) {
    ...
    RCLCPP_INFO(logger, "Ketchup auto squeeze pose: xyz=[%.3f %.3f %.3f]",
                plan.squeeze_pose.position.x, plan.squeeze_pose.position.y,
...
KetchupReturnPlacePlan makeKetchupReturnPlacePlan(const rclcpp::Logger& logger,
  ...
  PickPlan pick_plan =
      makeHorizontalPickPlan(ketchup_grasp_target, Eigen::Vector3d::UnitX(),
                             Eigen::Vector3d::UnitY(), ketchup_pre_grasp_distance_m, lift_height_m,
                             task_presets::kLeftKetchupPickTuning.grasp_tcp_z_offset_m);

  KetchupReturnPlacePlan plan;
  plan.return_pose = pick_plan.grasp_pose;

  if (const auto* pick_pre_grasp_preset = task_presets::findStageWaypointPosePreset(
          task_presets::ManufacturingTarget::Ketchup, task_presets::ArmSide::Left,
          task_presets::ManufacturingStage::Pick, "pre_grasp")) {
  ...
  if (std::abs(task_presets::kLeftKetchupPickWristRollDeg) > 1e-9) {
    ...
    RCLCPP_INFO(logger, "Ketchup return place uses pick-equivalent wrist roll correction: %.2f deg",
                task_presets::kLeftKetchupPickWristRollDeg);
  }
  plan.lift_pose.position.z += task_presets::kLeftKetchupReturnLiftHeightM;

  if (const auto* return_preset = task_presets::findStageWaypointPosePreset(
          task_presets::ManufacturingTarget::Ketchup, task_presets::ArmSide::Left,
          task_presets::ManufacturingStage::Place, "return_pose")) {
    plan.return_pose = makePoseFromPreset(return_preset->pose);
    plan.lift_pose = plan.return_pose;
    plan.lift_pose.position.z += task_presets::kLeftKetchupReturnLiftHeightM;
    RCLCPP_INFO(logger, "Ketchup return place pose preset applied");
  } else {
    RCLCPP_INFO(logger, "Ketchup auto return place pose: xyz=[%.3f %.3f %.3f]",
                plan.return_pose.position.x, plan.return_pose.position.y,
...
CompletedHotdogPlacePlan makeCompletedHotdogPlacePlan(
    const rclcpp::Logger& logger, const TargetObject& pickup_zone, const TargetObject& case_target,
  ...
  const Eigen::Vector3d desired_case_center(
      pickup_center.x(), pickup_center.y(),
  ...
  const Eigen::Vector3d release_tcp_position =
      desired_case_center - tcp_z * (-task_presets::kRightCasePickTuning.grasp_tcp_z_offset_m);
  ...
  if (const auto* release_pose_preset = task_presets::findStageWaypointPosePreset(
          task_presets::ManufacturingTarget::Hotdog, task_presets::ArmSide::Right,
          task_presets::ManufacturingStage::Place, "release_pose")) {
    ...
    plan.release_pose.orientation = plan.approach_pose.orientation;
    RCLCPP_INFO(logger, "Completed hotdog auto approach pose: xyz=[%.3f %.3f %.3f]",
                plan.approach_pose.position.x, plan.approach_pose.position.y,
                plan.approach_pose.position.z);
    RCLCPP_INFO(logger, "Completed hotdog auto release pose: xyz=[%.3f %.3f %.3f]",
                plan.release_pose.position.x, plan.release_pose.position.y,
...
BeverageHandoffPlan makeBeverageHandoffPlan(const rclcpp::Logger& logger,
  ...
  if (const auto* left_handoff_preset = task_presets::findStageWaypointPosePreset(
          beverage_target, task_presets::ArmSide::Left, task_presets::ManufacturingStage::Work,
          "handoff")) {
  ...
  PickPlan right_receive_plan = makeHorizontalPickPlan(
      TargetObject{
          "beverage_handoff", "",
          Eigen::Vector3d(plan.right_receive_pose.position.x, plan.right_receive_pose.position.y,
                          plan.right_receive_pose.position.z),
          Eigen::Vector3d::Zero(), Eigen::Vector3d::Zero(), beverage_target_object.size},
      -Eigen::Vector3d::UnitY(), Eigen::Vector3d::UnitX(), 0.0, 0.0,
  ...
  if (const auto* right_pre_receive_preset = task_presets::findStageWaypointPosePreset(
          beverage_target, task_presets::ArmSide::Right, task_presets::ManufacturingStage::Work,
          "pre_receive")) {
  ...
  if (plan.right_pre_receive_pose_preset_enabled) {
    ...
    RCLCPP_INFO(logger, "Beverage right receive pose centered on handoff can: xyz=[%.3f %.3f %.3f]",
                plan.right_receive_pose.position.x, plan.right_receive_pose.position.y,
                plan.right_receive_pose.position.z);
  }
...
BeveragePickupPlacePlan makeBeveragePickupPlacePlan(
  ...
  plan.release_pose.position.z = pickup_top_z + beverage_target_object.size.z() * 0.5 +
                                 task_presets::kBeveragePickupPlaceClearanceM +
                                 task_presets::kRightBeverageCanReceiveWorldZOffsetM;

  const auto* approach_preset = task_presets::findStageWaypointPosePreset(
      beverage_target, task_presets::ArmSide::Right, task_presets::ManufacturingStage::Place,
      "approach");
  plan.approach_preset_enabled = approach_preset != nullptr && approach_preset->pose.enabled;

  const auto* release_preset = task_presets::findStageWaypointPosePreset(
      beverage_target, task_presets::ArmSide::Right, task_presets::ManufacturingStage::Place,
      "release_pose");
  ...
  if (plan.release_preset_enabled) {
    ...
    PickPlan place_plan = makeHorizontalPickPlan(
        TargetObject{"beverage_pickup_place", "",
                     Eigen::Vector3d(plan.release_pose.position.x, plan.release_pose.position.y,
                                     plan.release_pose.position.z),
                     Eigen::Vector3d::Zero(), Eigen::Vector3d::Zero(), beverage_target_object.size},
        -Eigen::Vector3d::UnitY(), Eigen::Vector3d::UnitX(), 0.0, 0.0,
        task_presets::kRightBeverageCanReceiveTuning.grasp_tcp_z_offset_m);
