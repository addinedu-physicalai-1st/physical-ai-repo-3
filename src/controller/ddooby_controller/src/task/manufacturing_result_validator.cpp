...
bool extractQuotedValue(const std::string& text, const std::string& key, std::string& value) {
...
bool extractScalarValue(const std::string& text, const std::string& key, double& value) {
  const std::string marker = key + ":";
  const size_t begin = text.find(marker);
  if (begin == std::string::npos) {
    return false;
...
bool extractBlock(const std::string& text, const std::string& key, std::string& block) {
  const std::string marker = key + " {";
...
std::map<std::string, Eigen::Vector3d> parseGazeboPoseInfo(const std::string& text) {
  ...
  while (search_from < text.size()) {
    ...
    const std::string pose_block = text.substr(brace_begin + 1, pose_end - brace_begin - 1);
    std::string name;
    std::string position_block;
    if (extractQuotedValue(pose_block, "name", name) &&
        extractBlock(pose_block, "position", position_block)) {
      double x = 0.0;
      double y = 0.0;
      double z = 0.0;
...
void logModelPosition(const rclcpp::Logger& logger, const std::string& model_name,
                      const Eigen::Vector3d& position) {
  RCLCPP_INFO(logger, "Gazebo result pose: %s xyz=[%.3f %.3f %.3f]", model_name.c_str(),
              position.x(), position.y(), position.z());
}

bool validateDistance(const rclcpp::Logger& logger, const std::string& label, double distance,
                      double tolerance) {
  const bool ok = distance <= tolerance;
  if (ok) {
    RCLCPP_INFO(logger, "Gazebo result validation OK: %s distance=%.3fm tolerance=%.3fm",
                label.c_str(), distance, tolerance);
  } else {
    RCLCPP_ERROR(logger, "Gazebo result validation FAILED: %s distance=%.3fm tolerance=%.3fm",
                 label.c_str(), distance, tolerance);
  }
  return ok;
}

bool validateAbsDelta(const rclcpp::Logger& logger, const std::string& label, double delta,
                      double tolerance) {
  const bool ok = std::abs(delta) <= tolerance;
  if (ok) {
    RCLCPP_INFO(logger, "Gazebo result validation OK: %s delta=%.3fm tolerance=%.3fm",
                label.c_str(), delta, tolerance);
  } else {
    RCLCPP_ERROR(logger, "Gazebo result validation FAILED: %s delta=%.3fm tolerance=%.3fm",
                 label.c_str(), delta, tolerance);
  }
  return ok;
}

bool validateMinimumDistance(const rclcpp::Logger& logger, const std::string& label,
                             double distance, double minimum) {
  const bool ok = distance >= minimum;
  if (ok) {
    RCLCPP_INFO(logger, "Gazebo result validation OK: %s distance=%.3fm minimum=%.3fm",
                label.c_str(), distance, minimum);
  } else {
    RCLCPP_ERROR(logger, "Gazebo result validation FAILED: %s distance=%.3fm minimum=%.3fm",
                 label.c_str(), distance, minimum);
  }
  return ok;
}

bool validateInsideFootprint(const rclcpp::Logger& logger, const std::string& label,
                             const Eigen::Vector3d& position, const TargetObject& target,
                             double margin_m) {
  if (target.size.x() <= 1e-6 || target.size.y() <= 1e-6) {
    return validateDistance(logger, label, xyDistance(position, target.xyz), margin_m);
  }
  ...
  if (ok) {
    RCLCPP_INFO(logger, "Gazebo result validation OK: %s dx=%.3fm/%.3fm dy=%.3fm/%.3fm",
                label.c_str(), dx, max_x, dy, max_y);
  } else {
    RCLCPP_ERROR(logger, "Gazebo result validation FAILED: %s dx=%.3fm/%.3fm dy=%.3fm/%.3fm",
                 label.c_str(), dx, max_x, dy, max_y);
  }
  return ok;
}
...
ManufacturingResultValidator::ManufacturingResultValidator(
    const rclcpp::Logger& logger, ManufacturingResultValidatorConfig config)
    : logger_(logger), config_(std::move(config)) {}

bool ManufacturingResultValidator::shouldValidate() const {
  return config_.enabled && !config_.dry_run && !config_.has_partial_stage_limit &&
...
bool ManufacturingResultValidator::loadTarget(const std::string& target_model,
  ...
  try {
    ...
    RCLCPP_ERROR(logger_, "Failed to load target object '%s': %s", target_model.c_str(),
                 error.what());
    return false;
  }
  return true;
...
bool ManufacturingResultValidator::validateHotdogPlacement() {
  ...
  bool ok = true;
  ok &= validateDistance(logger_, config_.bread_target_model + " near " + config_.case_target_model,
                         xyDistance(bread_pose, case_pose), config_.hotdog_item_xy_tolerance_m);
  ok &=
      validateDistance(logger_, config_.sausage_target_model + " near " + config_.case_target_model,
                       xyDistance(sausage_pose, case_pose), config_.hotdog_item_xy_tolerance_m);
  ok &= validateDistance(logger_, config_.case_target_model + " near pickup_zone",
                         xyDistance(case_pose, pickup_zone_layout.xyz),
                         config_.beverage_pickup_xy_tolerance_m);
  ok &= validateDistance(logger_, config_.ketchup_target_model + " returned near layout pose",
                         xyDistance(ketchup_pose, ketchup_layout.xyz),
                         config_.ketchup_return_xy_tolerance_m);
  ok &= validateAbsDelta(logger_, config_.ketchup_target_model + " returned near layout height",
...
bool ManufacturingResultValidator::validateBeveragePlacement(
  ...
  const std::string beverage_model =
      beverageTargetModel(beverage_target, config_.target_model, config_.coffee_target_model,
                          config_.coke_target_model);
  TargetObject pickup_zone_layout;
  ...
  ok &= validateInsideFootprint(logger_, beverage_model + " inside pickup_zone footprint",
                                beverage_pose_it->second, pickup_zone_layout,
                                kPickupZoneFootprintMarginM);

  constexpr double kMinimumMovedDistanceM = 0.20;
  ok &= validateMinimumDistance(logger_, beverage_model + " moved from layout pose",
                                xyDistance(beverage_pose_it->second, beverage_layout.xyz),
                                kMinimumMovedDistanceM);

  if (!ok) {
    RCLCPP_ERROR(logger_, "Gazebo result validation failed for beverage %s serving",
                 task_presets::targetName(beverage_target));
    return false;
  }

  RCLCPP_INFO(logger_, "Gazebo result validation passed for beverage %s serving",
              task_presets::targetName(beverage_target));
  return true;
}
...
bool HotdogMakingNode::shouldValidateGazeboResult() const {
  return validate_gazebo_result_ && !dry_run_ && !has_play_to_stage_ && play_to_waypoint_.empty();
}
...
bool HotdogMakingNode::validateFinalHotdogPlacement() {
  ...
  ManufacturingResultValidator validator(
      ...
      ManufacturingResultValidatorConfig{
          validate_gazebo_result_, dry_run_, has_play_to_stage_, !play_to_waypoint_.empty(),
          result_validation_settle_ms_, result_validation_hotdog_item_xy_tolerance_m_,
          result_validation_ketchup_return_xy_tolerance_m_,
          result_validation_ketchup_return_z_tolerance_m_,
          result_validation_beverage_pickup_xy_tolerance_m_, package_share_directory, layout_path_,
          case_target_model_, bread_target_model_, sausage_target_model_, ketchup_target_model_,
          coke_target_model_, coffee_target_model_, target_model_});
