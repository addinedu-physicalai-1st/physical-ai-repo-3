...
bool HotdogMakingNode::prepareAndRunPickMotion(const PickMotionConfig& config,
                                               const TargetObject& target, PickPlan pick_plan) {
  ...
  applyPickStageWaypointPresets(get_logger(), config.target, config.arm, target, pre_grasp_height_,
                                lift_height_, pick_plan);
  ...
  RCLCPP_INFO(get_logger(), "%s MoveIt setup: constructing arm group '%s'",
              config.log_label.c_str(), config.arm_group.c_str());
  moveit::planning_interface::MoveGroupInterface arm(self, config.arm_group);
  RCLCPP_INFO(get_logger(), "%s MoveIt setup: arm group '%s' constructed", config.log_label.c_str(),
              config.arm_group.c_str());
  RCLCPP_INFO(get_logger(), "%s MoveIt setup: constructing gripper group '%s'",
              config.log_label.c_str(), config.gripper_group.c_str());
  moveit::planning_interface::MoveGroupInterface gripper(self, config.gripper_group);
  RCLCPP_INFO(get_logger(), "%s MoveIt setup: gripper group '%s' constructed",
              config.log_label.c_str(), config.gripper_group.c_str());
  ...
  RCLCPP_INFO(get_logger(), "%s MoveIt setup: planning_frame='%s', eef='%s' (%s)",
              config.log_label.c_str(), arm.getPlanningFrame().c_str(),
              arm.getEndEffectorLink().c_str(), tcp_set ? "ok" : "failed");
  ...
  const MotionStepResult pre_grasp_result = runPickPreGraspStage(
      config, tuning, pick_plan, pre_grasp_pose_configured, arm, gripper,
      gripper_opened_for_approach, pre_grasp_goal_mode, reached_pre_grasp_pose);
  ...
  limitPickLiftHeight(get_logger(), config.log_label, config.max_lift_height_m, grasp_pose,
                      lift_pose);

  if (shouldRunStage(ManufacturingStage::Pick)) {
    const MotionStepResult alignment_result =
        runPickTargetAlignment(config, pick_plan, grasp_pose, pre_grasp_pose_configured,
                               pick_pose_configured, arm, reached_pre_grasp_pose);
    ...
    const MotionStepResult grasp_result = runPickGraspAttachAndDirectLift(
        config, tuning, target, planning_scene_interface, arm, gripper, gripper_opened_for_approach,
        grasp_pose, lift_pose);
  ...
  if (config.target == ManufacturingTarget::Bread ||
...
MotionStepResult HotdogMakingNode::runPickHomeStage(
    const PickMotionConfig& config, moveit::planning_interface::MoveGroupInterface& arm) {
  ...
  if (!planAndExecuteStageWaypointPoseIfConfigured(
          get_logger(), arm, config.target, config.arm, task_presets::ManufacturingStage::Home,
          "ready", config.tcp_link, home_stage_configured, cartesian_min_duration_sec_)) {
  ...
  if (!home_stage_configured) {
    ...
    RCLCPP_INFO(get_logger(), "%s: moved arm using configured home/ready waypoint pose",
                config.log_label.c_str());
  }

  logCurrentTcpPose(get_logger(), arm, config.tcp_link, config.log_label + " ready");
  return shouldStopAfter(ManufacturingStage::Home) ? MotionStepResult::Stop
...
MotionStepResult HotdogMakingNode::runOptionalStageWaypoint(
  ...
  if (!planAndExecuteStageWaypointPoseIfConfigured(
          get_logger(), arm, config.target, config.arm, stage_waypoint.stage,
          stage_waypoint.waypoint, config.tcp_link, configured, cartesian_min_duration_sec_)) {
    return MotionStepResult::Failed;
  }

  if (configured) {
    logCurrentTcpPose(get_logger(), arm, config.tcp_link,
                      config.log_label + " " + stage_waypoint.log_suffix);
  }

  return shouldStopAfter(stage_waypoint.stage) ? MotionStepResult::Stop
                                               : MotionStepResult::Continue;
}
...
MotionStepResult HotdogMakingNode::runPickPreGraspStage(
    ...
    PreGraspGoalMode& pre_grasp_goal_mode, geometry_msgs::msg::Pose& reached_pre_grasp_pose) {
  const auto motions = makeMotionPrimitives();
  ...
  if (config.target == ManufacturingTarget::Case) {
    RCLCPP_INFO(get_logger(), "%s: opening gripper at ready pose before case approach",
                config.log_label.c_str());
  ...
  const bool use_pre_grasp_clearance =
      pre_grasp_pose_configured && (config.target == ManufacturingTarget::Bread ||
                                    config.target == ManufacturingTarget::Sausage ||
                                    config.target == ManufacturingTarget::Ketchup);
  if (!planAndExecutePreGrasp(get_logger(), arm, pick_plan.pre_grasp_pose, config.tcp_link,
                              use_pre_grasp_clearance, pose_min_duration_sec_,
                              pre_grasp_goal_mode)) {
    return MotionStepResult::Failed;
  }

  RCLCPP_INFO(get_logger(), "%s: reached pre-grasp using %s goal mode", config.log_label.c_str(),
...
MotionStepResult HotdogMakingNode::runPickGraspAttachAndDirectLift(
  ...
  auto restore_target_gripper_collision = [&]() {
    ...
    const bool restored = applyTargetGripperAllowedCollision(
        shared_from_this(), get_logger(), planning_scene_interface, target.name,
        gripper_touch_links, false, collision_scene_settle_ms_);
    target_gripper_collision_allowed = false;
    return restored;
  };

  if (!gripper_opened_for_approach) {
    RCLCPP_INFO(get_logger(), "%s: opening gripper after target alignment",
                config.log_label.c_str());
    if (!motions.gripperPickAction(gripper, config.log_label, tuning, GripperPickAction::Open)) {
      return MotionStepResult::Failed;
    }
  ...
  if (allow_gripper_target_collision_for_grasp_) {
    ...
    if (!applyTargetGripperAllowedCollision(
            shared_from_this(), get_logger(), planning_scene_interface, target.name,
            gripper_touch_links, true, collision_scene_settle_ms_)) {
      return MotionStepResult::Failed;
    }
  ...
  if (config.use_planned_grasp_approach) {
    ...
    if (!motions.moveToPose(arm, grasp_pose, config.tcp_link,
    ...
    if (!motions.cartesianMoveToPoseWithPlanningFallback(
            arm, grasp_pose, config.tcp_link, config.log_label + " grasp approach",
            config.log_label + " planned grasp approach",
            config.allow_planned_grasp_approach_fallback, cartesian_avoid_collisions_)) {
      restore_target_gripper_collision();
      return MotionStepResult::Failed;
    }
  }
  ...
  if (!attachTargetCollisionObject(get_logger(), arm, attached_collision_objects_, target.name,
                                   config.tcp_link, gripper_touch_links, config.log_label,
  ...
  if (!motions.cartesianMoveToPoseWithPlanningFallback(
          arm, lift_pose, config.tcp_link, config.log_label + " lift",
          config.log_label + " planned lift", true, cartesian_avoid_collisions_)) {
    return MotionStepResult::Failed;
  }
  logCurrentTcpPose(get_logger(), arm, config.tcp_link, config.log_label + " lift");
  return shouldStopAfterWaypoint(ManufacturingStage::Pick, "lift") ? MotionStepResult::Stop
                                                                   : MotionStepResult::Continue;
}

MotionStepResult HotdogMakingNode::runPickPullOutAndLift(
    const PickMotionConfig& config, const PickPlan& pick_plan,
    moveit::planning_interface::MoveGroupInterface& arm, geometry_msgs::msg::Pose& lift_pose) {
  const auto motions = makeMotionPrimitives();
  ...
  if (pull_out_preset != nullptr) {
    ...
    RCLCPP_INFO(get_logger(), "%s: pull-out waypoint pose preset applied",
                config.log_label.c_str());
  } else {
  ...
  if (!motions.cartesianMoveToPose(arm, pull_out_pose, config.log_label + " pull-out",
...
MotionStepResult HotdogMakingNode::runPickTargetAlignment(
    const PickMotionConfig& config, const PickPlan& pick_plan,
    const geometry_msgs::msg::Pose& grasp_pose, bool pre_grasp_pose_configured,
    bool pick_pose_configured, moveit::planning_interface::MoveGroupInterface& arm,
    geometry_msgs::msg::Pose& reached_pre_grasp_pose) {
  const auto motions = makeMotionPrimitives();
  ...
  RCLCPP_INFO(get_logger(),
              ...
              config.log_label.c_str(), pick_plan.pre_grasp_pose.position.x,
              pick_plan.pre_grasp_pose.position.y, reached_pre_grasp_pose.position.x,
              reached_pre_grasp_pose.position.y, pre_grasp_xy_error);

  const bool has_target_alignment_after_pre_grasp =
      pre_grasp_pose_configured && !pick_pose_configured &&
      (config.target == ManufacturingTarget::Case || config.target == ManufacturingTarget::Bread ||
       config.target == ManufacturingTarget::Sausage ||
       config.target == ManufacturingTarget::Ketchup);
  if (pre_grasp_xy_error > max_pre_grasp_xy_error_ && !has_target_alignment_after_pre_grasp) {
    RCLCPP_ERROR(get_logger(), "%s: rejecting pre-grasp because TCP xy error %.3f m exceeds %.3f m",
                 config.log_label.c_str(), pre_grasp_xy_error, max_pre_grasp_xy_error_);
    return MotionStepResult::Failed;
  }

  if (pre_grasp_xy_error > max_pre_grasp_xy_error_) {
    RCLCPP_WARN(get_logger(),
                "%s: pre-grasp TCP xy error %.3f m exceeds %.3f m; continuing to explicit "
                "target-alignment step",
                config.log_label.c_str(), pre_grasp_xy_error, max_pre_grasp_xy_error_);
  }
  ...
  if (config.target == ManufacturingTarget::Ketchup && pre_grasp_xy_error > 0.015) {
    ...
    RCLCPP_INFO(get_logger(),
                ...
                config.log_label.c_str());
    if (!motions.moveToPose(arm, target_aligned_pre_grasp_pose, config.tcp_link,
  ...
  if (config.target == ManufacturingTarget::Case) {
    ...
    RCLCPP_INFO(
        ...
        config.log_label.c_str(), task_presets::kRightCaseTargetAlignZOffsetM);
    if (!motions.moveToPose(arm, target_aligned_pre_grasp_pose, config.tcp_link,
  ...
  if (config.target == ManufacturingTarget::Sausage || config.target == ManufacturingTarget::Coke ||
  ...
  const double clearance_z =
      std::max({reached_pre_grasp_pose.position.z, pick_plan.pre_grasp_pose.position.z,
  ...
  if (std::abs(clearance_pose.position.z - reached_pre_grasp_pose.position.z) > 0.005) {
    RCLCPP_INFO(get_logger(),
                ...
                config.log_label.c_str(), clearance_z);
    if (!motions.moveToPose(arm, clearance_pose, config.tcp_link,
  ...
  if (config.target == ManufacturingTarget::Sausage) {
    ...
    RCLCPP_INFO(get_logger(), "%s: moving above selected target before vertical grasp descent",
                config.log_label.c_str());
  }
  if (!motions.moveToPose(arm, target_aligned_pre_grasp_pose, config.tcp_link,
