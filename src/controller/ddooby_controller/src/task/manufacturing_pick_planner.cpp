...
const task_presets::TcpPosePreset* findPullOutPosePreset(task_presets::ManufacturingTarget target,
  ...
  const auto* preset = task_presets::findStageWaypointPosePreset(
      target, arm, task_presets::ManufacturingStage::Pick, "pull_out");
  if (preset != nullptr) {
    return &preset->pose;
  }
  return nullptr;
...
void applyStageWaypointPosePreset(const rclcpp::Logger& logger,
                                  task_presets::ManufacturingTarget target,
                                  task_presets::ArmSide arm, task_presets::ManufacturingStage stage,
                                  const char* waypoint, geometry_msgs::msg::Pose& pose,
  ...
  const std::string label = std::string(task_presets::stageName(stage)) + "." + waypoint;
...
TargetObject loadTargetObject(const std::string& package_share_directory,
  ...
  const std::string sdf_path = joinPath(joinPath(package_share_directory, "assets"),
                                        joinPath(object.model_dir, "model.sdf"));
  const std::vector<CollisionBox> collision_boxes = parseCollisionBoxes(readTextFile(sdf_path));
  if (collision_boxes.empty()) {
    throw std::runtime_error("target model '" + target_model +
                             "' has no supported collision geometry");
  }
...
PickPlan makeTopDownPickPlan(const TargetObject& target, double pre_grasp_height,
  ...
  return PickPlan{makePose(pre_grasp_position, tcp_orientation),
                  makePose(grasp_position, tcp_orientation),
                  makePose(lift_position, tcp_orientation), principal_axis, closing_axis};
}
...
PickPlan makeHorizontalPickPlan(const TargetObject& target, const Eigen::Vector3d& approach_axis,
  ...
  return PickPlan{makePose(pre_grasp_position, tcp_orientation),
                  makePose(grasp_position, tcp_orientation),
                  makePose(lift_position, tcp_orientation), tcp_z, tcp_y};
}
...
void applyPickStageWaypointPresets(const rclcpp::Logger& logger,
                                   ...
                                   PickPlan& pick_plan) {
  const bool pre_grasp_pose_configured = hasStageWaypointPosePreset(
      target_kind, arm, task_presets::ManufacturingStage::Pick, "pre_grasp");
  ...
  if (pre_grasp_pose_configured) {
    applyStageWaypointPosePreset(logger, target_kind, arm, task_presets::ManufacturingStage::Pick,
                                 "pre_grasp", pick_plan.pre_grasp_pose, pick_axis_references);
  }
  if (pick_pose_configured) {
    applyStageWaypointPosePreset(logger, target_kind, arm, task_presets::ManufacturingStage::Pick,
                                 "grasp", pick_plan.grasp_pose, pick_axis_references);
  }
  ...
  if (target_kind == task_presets::ManufacturingTarget::Ketchup &&
    ...
    RCLCPP_INFO(logger, "Ketchup pick final pose wrist roll correction applied: %.2f deg",
...
void logPickPlanSummary(const rclcpp::Logger& logger, const std::string& log_label,
  ...
  RCLCPP_INFO(logger,
              "Target '%s': xyz=[%.3f %.3f %.3f], size=[%.3f %.3f %.3f], principal=[%.3f %.3f "
              "%.3f], closing=[%.3f %.3f %.3f]",
              target.name.c_str(), target.xyz.x(), target.xyz.y(), target.xyz.z(), target.size.x(),
              target.size.y(), target.size.z(), pick_plan.principal_axis.x(),
              pick_plan.principal_axis.y(), pick_plan.principal_axis.z(),
              pick_plan.closing_axis.x(), pick_plan.closing_axis.y(), pick_plan.closing_axis.z());
  RCLCPP_INFO(logger, "%s tuning: grasp_tcp_z_offset=%.3f, gripper_joint_position=%s%.3f",
              log_label.c_str(), tuning.grasp_tcp_z_offset_m,
...
void limitPickLiftHeight(const rclcpp::Logger& logger, const std::string& log_label,
  ...
  RCLCPP_INFO(logger, "%s: limiting pick lift height %.3f -> %.3f m", log_label.c_str(),
              requested_lift, max_lift_height_m);
}
...
void logDryRunPickPlan(const rclcpp::Logger& logger, const std::string& log_label,
                       const PickPlan& pick_plan) {
  RCLCPP_INFO(logger,
              "%s dry run completed: pre_grasp=[%.3f %.3f %.3f], grasp=[%.3f %.3f %.3f], "
              "lift=[%.3f %.3f %.3f]",
              log_label.c_str(), pick_plan.pre_grasp_pose.position.x,
              pick_plan.pre_grasp_pose.position.y, pick_plan.pre_grasp_pose.position.z,
              pick_plan.grasp_pose.position.x, pick_plan.grasp_pose.position.y,
              pick_plan.grasp_pose.position.z, pick_plan.lift_pose.position.x,
              pick_plan.lift_pose.position.y, pick_plan.lift_pose.position.z);
}
