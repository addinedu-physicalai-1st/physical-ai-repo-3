...
bool shouldStopAtOrBeforeStage(bool has_play_to_stage,
                               ...
                               task_presets::ManufacturingStage stage) {
  return has_play_to_stage && stageOrder(play_to_stage) <= stageOrder(stage);
}
...
bool shouldStopAtOrBeforeWaypointStage(const std::string& play_to_waypoint,
  ...
  for (const auto candidate_stage :
       {task_presets::ManufacturingStage::Home, task_presets::ManufacturingStage::Pick,
        task_presets::ManufacturingStage::Work, task_presets::ManufacturingStage::Place,
...
bool stopWaypointMatches(const std::string& play_to_waypoint,
  ...
  return play_to_waypoint == normalized_waypoint || play_to_waypoint == normalized_stage_waypoint;
}

std::vector<ScenarioStep> makeHotdogScenarioSteps() {
  return {
      {"case_pull",
       "right arm pulls the New York hotdog case horizontally from the right-side stack"},
      {"bread_pick",
       "right arm keeps holding the case while left arm picks bread from the handled bread tray"},
      ...
      {"ketchup_squeeze",
       "left gripper slightly closes and moves horizontally along the sausage length"},
