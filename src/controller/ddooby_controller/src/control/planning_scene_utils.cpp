...
moveit_msgs::msg::CollisionObject makeCollisionObjectFromSdf(
  ...
  for (const CollisionPrimitiveSpec& primitive_spec : primitives) {
    ...
    const Eigen::Vector3d global_center = model_xyz + model_rotation * primitive_spec.center;
    const Eigen::Matrix3d global_rotation = model_rotation * rotationFromRpy(primitive_spec.rpy);
...
moveit_msgs::msg::CollisionObject makeCollisionObjectFromTargetObject(
  ...
  const std::string sdf_path = joinPath(joinPath(package_share_directory, "assets"),
                                        joinPath(target.model_dir, "model.sdf"));
  const std::vector<CollisionPrimitiveSpec> primitives =
      parseCollisionPrimitives(readTextFile(sdf_path));
  if (primitives.empty()) {
    throw std::runtime_error("target model '" + target.name +
                             "' has no supported collision geometry");
  }
  ...
  for (const CollisionPrimitiveSpec& primitive_spec : primitives) {
    ...
    const Eigen::Vector3d global_center = target.xyz + model_rotation * primitive_spec.center;
    const Eigen::Matrix3d global_rotation = model_rotation * rotationFromRpy(primitive_spec.rpy);
...
std::vector<std::string> makeGripperTouchLinks(
  ...
  if (tcp_link.size() > kHandTcpSuffix.size() &&
      tcp_link.compare(tcp_link.size() - kHandTcpSuffix.size(), kHandTcpSuffix.size(),
                       kHandTcpSuffix) == 0) {
...
bool applyTargetGripperAllowedCollision(
  ...
  RCLCPP_INFO(logger, "%s target collision between '%s' and %zu links",
              allow ? "Allowing" : "Disallowing", target_object.c_str(), touch_links.size());
  if (!planning_scene_interface.applyPlanningScene(scene)) {
    RCLCPP_ERROR(logger, "Failed to apply allowed collision update for target '%s'",
                 target_object.c_str());
    return false;
  }
  if (settle_ms > 0) {
...
bool attachTargetCollisionObject(const rclcpp::Logger& logger,
  ...
  if (attached_collision_objects.find(target_model) != attached_collision_objects.end()) {
    RCLCPP_INFO(logger, "%s: collision object '%s' is already attached", log_label.c_str(),
                target_model.c_str());
    return true;
  }

  RCLCPP_INFO(logger, "%s: attaching collision object '%s' to '%s' with %zu touch links",
              log_label.c_str(), target_model.c_str(), attach_link.c_str(), touch_links.size());
  if (!arm.attachObject(target_model, attach_link, touch_links)) {
    RCLCPP_ERROR(logger, "%s: failed to attach collision object '%s'", log_label.c_str(),
                 target_model.c_str());
    return false;
  }
...
bool detachTargetCollisionObject(const rclcpp::Logger& logger,
  ...
  if (attached_collision_objects.find(target_model) == attached_collision_objects.end()) {
    RCLCPP_INFO(logger, "%s: collision object '%s' is not attached; skipping detach",
                log_label.c_str(), target_model.c_str());
    return true;
  }

  RCLCPP_INFO(logger, "%s: detaching collision object '%s'", log_label.c_str(),
              target_model.c_str());
  if (!arm.detachObject(target_model)) {
    RCLCPP_ERROR(logger, "%s: failed to detach collision object '%s'", log_label.c_str(),
                 target_model.c_str());
    return false;
  }
...
bool removeTargetCollisionObject(
  ...
  RCLCPP_INFO(logger, "%s: removing world collision object '%s'", log_label.c_str(),
              target_model.c_str());
...
bool applyVisionTargetCollisionObject(
  ...
  try {
    ...
    RCLCPP_INFO(logger,
                ...
                log_label.c_str(), collision_object.id.c_str(), collision_object.primitives.size());
    planning_scene_interface.applyCollisionObject(collision_object);
    if (settle_ms > 0) {
      rclcpp::sleep_for(std::chrono::milliseconds(settle_ms));
    }
  } catch (const std::exception& error) {
    RCLCPP_ERROR(logger, "%s: failed to update vision collision object '%s': %s", log_label.c_str(),
...
bool restoreTargetCollisionObjectFromLayout(
  ...
  try {
    ...
    RCLCPP_INFO(logger, "%s: restoring collision object '%s' with %zu primitive(s)",
                log_label.c_str(), collision_object.id.c_str(), collision_object.primitives.size());
    planning_scene_interface.applyCollisionObject(collision_object);
    if (settle_ms > 0) {
      rclcpp::sleep_for(std::chrono::milliseconds(settle_ms));
    ...
    RCLCPP_ERROR(logger, "%s: failed to restore collision object for '%s': %s", log_label.c_str(),
                 target_model.c_str(), error.what());
    return false;
  }
