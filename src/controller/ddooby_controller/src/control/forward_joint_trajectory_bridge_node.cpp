...
trajectory_msgs::msg::JointTrajectoryPoint makePoint(const std::vector<double>& positions,
...
class ForwardJointTrajectoryBridge : public rclcpp::Node {
 public:
  ForwardJointTrajectoryBridge() : Node("forward_joint_trajectory_bridge") {
    controller_name_ =
        declare_parameter<std::string>("controller_name", "joint_trajectory_controller");
    command_topic_ =
        declare_parameter<std::string>("command_topic", "/forward_position_controller/commands");
    ...
    pre_hold_before_trajectory_sec_ =
        declare_parameter<double>("pre_hold_before_trajectory_sec", 0.15);
    ...
    if (hold_when_idle_) {
      const auto period =
          std::chrono::duration<double>(1.0 / std::max(1.0, idle_hold_publish_rate_hz_));
      idle_hold_timer_ =
    ...
    action_server_ = rclcpp_action::create_server<FollowJointTrajectory>(
        this, action_name,
        std::bind(&ForwardJointTrajectoryBridge::handleGoal, this, std::placeholders::_1,
                  std::placeholders::_2),
    ...
    RCLCPP_INFO(get_logger(),
                "Forward trajectory bridge ready: action=%s, command_topic=%s, joints=%zu, "
                "rate=%.1f, goal_tol=%.3f",
 ...
 private:
  ...
  bool validateGoal(const FollowJointTrajectory::Goal& goal, std::string& error) const {
    ...
    for (const auto& joint_name : joint_names_) {
      if (std::find(goal.trajectory.joint_names.begin(), goal.trajectory.joint_names.end(),
                    joint_name) == goal.trajectory.joint_names.end()) {
  ...
  std::vector<double> reorderPoint(const trajectory_msgs::msg::JointTrajectoryPoint& point,
  ...
  void publishFeedback(const std::shared_ptr<GoalHandle>& goal_handle,
                       const std::vector<double>& desired, double elapsed_sec) {
  ...
  bool waitForGoalSettled(const std::shared_ptr<GoalHandle>& goal_handle,
                          const std::vector<double>& goal_positions, rclcpp::Rate& rate) {
    ...
    while (rclcpp::ok()) {
      ...
      publishFeedback(goal_handle, goal_positions,
  ...
  void execute(const std::shared_ptr<GoalHandle> goal_handle) {
    ...
    for (const auto& joint_name : joint_names_) {
      const auto it = std::find(goal->trajectory.joint_names.begin(),
                                goal->trajectory.joint_names.end(), joint_name);
      goal_indices.push_back(
          static_cast<std::size_t>(std::distance(goal->trajectory.joint_names.begin(), it)));
    }
      ...
      if (first_point && skip_zero_time_start_point_ && target_time <= 1e-6) {
        ...
        if (start_jump > start_point_jump_warn_rad_) {
          RCLCPP_WARN(get_logger(),
                      "Skipping zero-time trajectory start point: max joint delta %.4f rad from "
                      "current state",
                      start_jump);
        }
