...
ManufacturingMotionPrimitives::ManufacturingMotionPrimitives(const rclcpp::Logger& logger,
...
bool ManufacturingMotionPrimitives::moveToNamed(
    moveit::planning_interface::MoveGroupInterface& group, const std::string& target_name,
    const std::string& label, double min_duration_sec) const {
  return planAndExecuteNamedTarget(logger_, group, target_name, label, config_.pose_max_attempts,
                                   poseMinDuration(min_duration_sec));
}

bool ManufacturingMotionPrimitives::moveToPose(moveit::planning_interface::MoveGroupInterface& arm,
                                               const geometry_msgs::msg::Pose& target_pose,
                                               const std::string& tcp_link,
                                               const std::string& label,
                                               double min_duration_sec) const {
  return planAndExecutePoseTarget(logger_, arm, target_pose, tcp_link, label,
                                  config_.pose_max_attempts, poseMinDuration(min_duration_sec));
}
...
bool ManufacturingMotionPrimitives::moveToPoseWithScaling(
  ...
  return planAndExecutePoseTargetWithScopedScaling(
      logger_, arm, target_pose, tcp_link, label, current_velocity_scaling,
      current_acceleration_scaling, scoped_velocity_scaling, scoped_acceleration_scaling,
      config_.pose_max_attempts, poseMinDuration(min_duration_sec));
}

bool ManufacturingMotionPrimitives::cartesianMoveToPose(
    moveit::planning_interface::MoveGroupInterface& arm,
    const geometry_msgs::msg::Pose& target_pose, const std::string& label, bool avoid_collisions,
    double min_duration_sec) const {
...
bool ManufacturingMotionPrimitives::cartesianMove(
    moveit::planning_interface::MoveGroupInterface& arm,
    const std::vector<geometry_msgs::msg::Pose>& waypoints, const std::string& label,
    bool avoid_collisions, double min_duration_sec) const {
  return executeCartesian(
      logger_, arm, waypoints, label, config_.cartesian_eef_step, config_.cartesian_min_fraction,
      avoid_collisions, config_.cartesian_velocity_scaling, config_.cartesian_acceleration_scaling,
...
bool ManufacturingMotionPrimitives::cartesianMoveToPoseWithPlanningFallback(
    ...
    bool allow_planned_fallback, bool avoid_collisions, double planned_min_duration_sec) const {
  return executeCartesianPoseWithPlanningFallback(
      logger_, arm, target_pose, tcp_link, cartesian_label, planned_label, allow_planned_fallback,
      config_.cartesian_eef_step, config_.cartesian_min_fraction, avoid_collisions,
      config_.cartesian_velocity_scaling, config_.cartesian_acceleration_scaling,
      config_.cartesian_min_duration_sec, poseMinDuration(planned_min_duration_sec),
      config_.pose_max_attempts);
}

bool ManufacturingMotionPrimitives::clearance(moveit::planning_interface::MoveGroupInterface& arm,
                                              const std::string& tcp_link, const std::string& label,
                                              const Eigen::Vector3d& direction, double distance_m,
...
bool ManufacturingMotionPrimitives::moveRelative(
    ...
    const std::string& label, const Eigen::Vector3d& direction, double distance_m,
    bool avoid_collisions, double min_duration_sec) const {
  const double direction_norm = direction.norm();
  if (direction_norm <= 1e-9) {
...
bool ManufacturingMotionPrimitives::gripperNamedTarget(
    moveit::planning_interface::MoveGroupInterface& gripper, const std::string& target_name,
...
bool ManufacturingMotionPrimitives::gripperPickAction(
    moveit::planning_interface::MoveGroupInterface& gripper, const std::string& label,
    const task_presets::PickTuningPreset& tuning, GripperPickAction action) const {
  switch (action) {
    case GripperPickAction::Open:
      return openGripperForPickApproach(logger_, gripper, label, tuning,
                                        config_.gripper_open_target);
    case GripperPickAction::Close:
      return closeGripperForPick(logger_, gripper, label, tuning, config_.gripper_grasp_target);
  }
  return false;
}
...
bool ManufacturingMotionPrimitives::gripperJointPosition(
    moveit::planning_interface::MoveGroupInterface& gripper, const std::string& label,
    const std::string& action, double joint_position) const {
  return moveGripperToJointPosition(logger_, gripper, label, action, joint_position);
}
