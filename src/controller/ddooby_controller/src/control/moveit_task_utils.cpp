...
void configureGripperGroupForTask(
...
void rememberJointTargetIfConfigured(
...
bool planAndExecuteNamedTarget(
  const rclcpp::Logger & logger,
  moveit::planning_interface::MoveGroupInterface & group,
  const std::string & target_name,
  const std::string & label,
  ...
  double min_duration_sec)
{
  ...
  RCLCPP_INFO(
    logger,
    "%s scoped scaling: velocity=%.3f acceleration=%.3f",
    label.c_str(),
    scoped_velocity_scaling,
    scoped_acceleration_scaling);

  const bool ok = planAndExecutePoseTarget(
    logger,
    arm,
    target_pose,
    tcp_link,
  ...
  double min_duration_sec)
{
  ...
  if (!retry_with_pose_target) {
    RCLCPP_ERROR(
      logger,
      "%s TCP is %.3f m / %.3f rad from target after execution",
      label.c_str(),
      position_error,
      orientation_error);
    return false;
  ...
  if (retry_error > position_tolerance_m ||
    retry_orientation_error > orientation_tolerance_rad)
  {
    RCLCPP_ERROR(
      logger,
      "%s TCP remains %.3f m / %.3f rad from target after retry",
      label.c_str(),
      retry_error,
      retry_orientation_error);
    return false;
  }
  return true;
}
  ...
  int max_attempts)
{
  ...
  RCLCPP_WARN(
    logger,
    "%s failed; trying regular pose planning as '%s'",
    cartesian_label.c_str(),
    planned_label.c_str());
