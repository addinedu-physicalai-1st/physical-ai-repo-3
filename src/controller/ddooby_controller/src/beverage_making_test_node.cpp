...
geometry_msgs::msg::Pose rotatePoseLocal(
  const geometry_msgs::msg::Pose & pose, const std::string & axis, double angle_rad)
{
...
double computePourAngleTowardTarget(
  const geometry_msgs::msg::Pose & pour_pose,
  const geometry_msgs::msg::Pose & target_pose,
  const Eigen::Vector3d & horizontal_lateral_vector,
  double configured_angle)
{
  const Eigen::Vector3d to_target(
    target_pose.position.x - pour_pose.position.x,
    target_pose.position.y - pour_pose.position.y,
    0.0);
...
geometry_msgs::msg::Pose verticalStickGraspPose(
  const geometry_msgs::msg::Pose & stick_pose,
  double grasp_height_offset)
{
...
geometry_msgs::msg::Pose stickEndGraspPose(
  const geometry_msgs::msg::Pose & stick_pose,
  const Eigen::Vector3d & horizontal_approach_vector,
  double stick_length,
  double grasp_inset)
{
  const Eigen::Quaterniond stick_orientation(
    stick_pose.orientation.w,
    stick_pose.orientation.x,
    stick_pose.orientation.y,
    stick_pose.orientation.z);
  Eigen::Vector3d stick_axis =
    stick_orientation.normalized().toRotationMatrix().col(0);
  stick_axis.z() = 0.0;
  ...
  const double end_distance =
    std::max(0.0, stick_length * 0.5 - std::max(0.0, grasp_inset));
...
geometry_msgs::msg::Pose graspPointPoseForProfile(
  ...
  const ObjectGraspProfile & profile,
  const Eigen::Vector3d & horizontal_approach_vector)
{
  switch (profile.point_mode) {
    ...
    case GraspPointMode::HorizontalStickEnd:
      return stickEndGraspPose(
        object_pose,
        horizontal_approach_vector,
        profile.stick_length,
...
std::optional<geometry_msgs::msg::Pose> tcpPoseForCupPose(
  ...
  const geometry_msgs::msg::Pose & desired_cup_pose)
{
  if (!context.tcp_to_cup_transform.has_value()) {
    return std::nullopt;
  }
  return toPoseMsg(
    poseToIsometry(desired_cup_pose) *
  ...
  const geometry_msgs::msg::Quaternion & tcp_orientation)
{
  ...
  const Eigen::Quaterniond tcp_rotation(
    ...
    tcp_orientation.y,
    tcp_orientation.z);
  const Eigen::Vector3d tcp_to_cup =
    context.tcp_to_cup_transform.value().translation();
  const Eigen::Vector3d desired_cup_position(
    desired_cup_pose.position.x,
    desired_cup_pose.position.y,
    desired_cup_pose.position.z);
...
public:
  ArmTask(
    ...
    ArmConfig config,
    TaskParams params)
  : node_(node), arm_(arm), gripper_(gripper), config_(std::move(config)), params_(std::move(params))
  {
  ...
  std::optional<ArmContext> computeContext(
    const geometry_msgs::msg::Pose & cup_pose,
    ...
    const ObjectGraspProfile & grasp_profile)
  {
    ...
    if (!target_state) {
      RCLCPP_ERROR(node_->get_logger(), "[%s] Failed to get current robot state", config_.label.c_str());
      return std::nullopt;
    }
    ...
    const Eigen::Vector3d tcp_approach_vector =
      grasp_transform.rotation().col(2).normalized();
    Eigen::Vector3d horizontal_approach_vector(
      tcp_approach_vector.x(), tcp_approach_vector.y(), 0.0);
    ...
    const Eigen::Vector3d horizontal_lateral_vector(
      -horizontal_approach_vector.y(), horizontal_approach_vector.x(), 0.0);
    ...
    mixing_hover_pose.position.x =
      mixing_cup_pose.position.x +
      horizontal_lateral_vector.x() * side_pour_target_lateral_offset -
      horizontal_approach_vector.x() * gripper_forward_offset;
    mixing_hover_pose.position.y =
      mixing_cup_pose.position.y +
      horizontal_lateral_vector.y() * side_pour_target_lateral_offset -
      horizontal_approach_vector.y() * gripper_forward_offset;
    mixing_hover_pose.position.z =
      mixing_cup_pose.position.z + params_.mixing_hover_height_offset;

    RCLCPP_INFO(
      node_->get_logger(),
      "[%s/%s] object [%.3f, %.3f, %.3f], grasp point [%.3f, %.3f, %.3f] (%s), tcp grasp [%.3f, %.3f, %.3f], pre-grasp [%.3f, %.3f, %.3f], finger front %.3f m, inward %.3f m, effective %.3f m",
      config_.label.c_str(),
      grasp_profile.name.c_str(),
      cup_pose.position.x,
      cup_pose.position.y,
      cup_pose.position.z,
      grasp_point_pose.position.x,
      grasp_point_pose.position.y,
      grasp_point_pose.position.z,
      graspPointModeName(grasp_profile.point_mode),
      tcp_grasp_pose.position.x,
      tcp_grasp_pose.position.y,
      tcp_grasp_pose.position.z,
      pre_grasp_pose.position.x,
      pre_grasp_pose.position.y,
      pre_grasp_pose.position.z,
      measured_gripper_forward_offset,
      grasp_profile.inward_offset,
      gripper_forward_offset);
  ...
  bool pickAndLift(
    const ArmContext & context,
    bool use_cartesian_approach = true,
    const std::string & gripper_target_override = "")
  {
    ...
    if (use_cartesian_approach) {
      ...
      RCLCPP_INFO(
        node_->get_logger(),
        "[%s] Pick phase: planned approach to grasp pose",
        config_.label.c_str());
      arm_.clearPoseTargets();
      setBoundedStartState(arm_);
    ...
    const std::string gripper_target =
      gripper_target_override.empty() ? params_.cup_grasp_profile.gripper_target :
      gripper_target_override;
    RCLCPP_INFO(
      node_->get_logger(),
      "[%s] Pick phase: close gripper to %s",
      config_.label.c_str(),
  ...
  bool approachGraspOnly(const ArmContext & context, bool use_cartesian_approach = false)
  {
    RCLCPP_INFO(
      node_->get_logger(),
      "[%s] Approach-only phase: ready -> pre-grasp",
      config_.label.c_str());
    arm_.setNamedTarget(config_.ready_pose_name);
    if (!planAndExecute(arm_, "ready pose")) {
      return false;
    ...
    RCLCPP_INFO(node_->get_logger(), "[%s] Approach-only phase: open gripper", config_.label.c_str());
    gripper_.setNamedTarget("open");
    if (!planAndExecute(gripper_, "open gripper")) {
      return false;
    }
  ...
  bool liftOnly(const ArmContext & context)
  {
    return executeCartesian({context.lift_pose}, "lift");
  }
    ...
    const std::vector<double> & guide_joint_waypoints = {})
  {
    RCLCPP_INFO(
      node_->get_logger(),
      "[%s] %s: direct target pose attempt",
      config_.label.c_str(),
      label.c_str());
    ...
    for (size_t candidate_index = 0; candidate_index < dynamic_candidates.size(); ++candidate_index) {
        ...
        ++waypoint_index)
      {
        const std::string waypoint_label =
          label + " dynamic " + std::to_string(candidate_index + 1) +
    ...
    RCLCPP_ERROR(
      node_->get_logger(),
      "[%s] %s: all pose fallback attempts failed",
      config_.label.c_str(),
      label.c_str());
    return false;
  }
    ...
    const std::string & label)
  {
    ...
    if (joints.size() != joint_names.size()) {
      RCLCPP_ERROR(
        node_->get_logger(),
        "[%s] %s has %zu joint values, but group '%s' expects %zu",
        config_.label.c_str(),
        label.c_str(),
        joints.size(),
        arm_.getName().c_str(),
  ...
  bool moveThroughJointValueWaypoints(
    const std::vector<double> & flat_joint_waypoints,
    const std::string & label)
  {
    ...
    for (size_t i = 0; i < waypoint_count; ++i) {
      const auto begin = flat_joint_waypoints.begin() + static_cast<std::ptrdiff_t>(i * joint_count);
  ...
  {
    return arm_.getJointNames().size();
  }
    ...
    const std::vector<double> & guide_joint_waypoints)
  {
    if (!guide_joint_waypoints.empty()) {
      RCLCPP_INFO(
        node_->get_logger(),
        "[%s] %s: trying configured home guide joint waypoints",
        config_.label.c_str(),
        label.c_str());
      if (moveThroughJointValueWaypoints(guide_joint_waypoints, label + " guide") &&
      ...
      RCLCPP_WARN(
        node_->get_logger(),
        "[%s] %s: configured home guide failed; trying direct home",
        config_.label.c_str(),
        label.c_str());
    }
  ...
  bool pourPlaceHome(
    ...
    const geometry_msgs::msg::Pose & mixing_cup_pose)
  {
    return pourAtMixing(context, mixing_cup_pose) &&
           clearAfterPour(context, mixing_cup_pose) &&
           placeHome(context);
  }
  ...
  bool pourAtMixing(
    ...
    const geometry_msgs::msg::Pose & mixing_cup_pose)
  {
    RCLCPP_INFO(node_->get_logger(), "[%s] Pour phase: move to mixing hover", config_.label.c_str());
    ...
    const double dynamic_pour_angle = computePourAngleTowardTarget(
      context.mixing_hover_pose,
      mixing_cup_pose,
      context.horizontal_lateral_vector,
      params_.pour_angle);
    RCLCPP_INFO(
      node_->get_logger(),
      "[%s] Dynamic pour angle: %.3f rad",
      config_.label.c_str(),
      dynamic_pour_angle);
    std::vector<geometry_msgs::msg::Pose> pour_waypoints;
    for (int i = 1; i <= params_.pour_waypoint_count; ++i) {
      const double ratio = static_cast<double>(i) / static_cast<double>(params_.pour_waypoint_count);
      pour_waypoints.push_back(
        rotatePoseLocal(context.mixing_hover_pose, params_.pour_axis, dynamic_pour_angle * ratio));
    }
  ...
  bool clearAfterPour(
    ...
    const geometry_msgs::msg::Pose & mixing_cup_pose)
  {
    RCLCPP_INFO(node_->get_logger(), "[%s] Place phase: return above original place", config_.label.c_str());
    ...
    if (!executeCartesian(
        {post_pour_clearance_pose, return_clearance_pose, corrected_lift_pose},
        "return hover"))
    {
      return false;
    }
...
private:
    ...
    const std::string & label)
  {
    ...
    const double fraction = arm_.computeCartesianPath(
      waypoints,
      params_.cartesian_eef_step,
      trajectory,
      params_.cartesian_avoid_collisions);

    RCLCPP_INFO(
      node_->get_logger(),
      "[%s] %s Cartesian path fraction: %.3f",
      config_.label.c_str(),
      label.c_str(),
      fraction);
    ...
    if (fraction < params_.min_cartesian_fraction) {
      RCLCPP_ERROR(
        ...
        label.c_str(),
        fraction,
        params_.min_cartesian_fraction);
      return false;
    }
    ...
    if (arm_.execute(plan) != moveit::core::MoveItErrorCode::SUCCESS) {
      RCLCPP_ERROR(
        node_->get_logger(),
        "[%s] Failed to execute %s Cartesian path",
        config_.label.c_str(),
        label.c_str());
      return false;
    }
  ...
  void logFingerMidpointOffset(
    const moveit::core::RobotState & state,
    const Eigen::Isometry3d & tcp_transform) const
  {
    const std::string suffix = "hand_tcp";
    if (config_.tcp_link.size() < suffix.size() ||
      config_.tcp_link.compare(config_.tcp_link.size() - suffix.size(), suffix.size(), suffix) != 0)
    {
      return;
    }

    const std::string prefix =
      config_.tcp_link.substr(0, config_.tcp_link.size() - suffix.size());
    ...
    if (
      robot_model->getLinkModel(left_finger_link) == nullptr ||
      robot_model->getLinkModel(right_finger_link) == nullptr)
    {
      RCLCPP_WARN(
        node_->get_logger(),
        "[%s] Cannot find finger links '%s' and '%s' in robot model",
        config_.label.c_str(),
        left_finger_link.c_str(),
        right_finger_link.c_str());
      return;
    }

    const Eigen::Vector3d midpoint =
      0.5 *
      (state.getGlobalLinkTransform(left_finger_link).translation() +
    ...
    RCLCPP_INFO(
      ...
      config_.label.c_str(),
      tcp_to_midpoint_local.x(),
      tcp_to_midpoint_local.y(),
    ...
    const Eigen::Isometry3d & tcp_transform) const
  {
    ...
    const std::string prefix =
      config_.tcp_link.substr(0, config_.tcp_link.size() - suffix.size());
    const std::array<std::string, 2> finger_links{
      prefix + "left_finger",
      prefix + "right_finger"};
    ...
    RCLCPP_INFO(
      node_->get_logger(),
      "[%s] TCP -> finger collision front along TCP +Z: %.3f m",
      config_.label.c_str(),
      max_forward);
    return max_forward;
  }
  ...
  const moveit::planning_interface::MoveGroupInterface::Plan & plan)
{
  ...
  RCLCPP_ERROR(node->get_logger(), "Failed to execute %s; retrying from refreshed state", label.c_str());
  rclcpp::sleep_for(std::chrono::milliseconds(500));
  setBoundedStartState(group);
  ...
  const std::vector<double> trajectory_msgs::msg::JointTrajectoryPoint::* field)
{
  ...
  for (size_t i = 1; i < trajectory.points.size(); ++i) {
    ...
    const double ratio = std::clamp((sample_time - previous_time) / (next_time - previous_time), 0.0, 1.0);
    std::vector<double> values;
    values.reserve(next_values.size());
    for (size_t value_index = 0; value_index < next_values.size(); ++value_index) {
      values.push_back(
        previous_values[value_index] +
  ...
  moveit_msgs::msg::RobotTrajectory & merged)
{
  ...
  output.joint_names.insert(output.joint_names.end(), left.joint_names.begin(), left.joint_names.end());
  output.joint_names.insert(output.joint_names.end(), right.joint_names.begin(), right.joint_names.end());
  output.points.clear();
  ...
  const double merged_duration = std::max(
    std::max(left_duration, right_duration),
    0.05 * static_cast<double>(point_count));
  ...
  for (size_t i = 0; i <= point_count; ++i) {
    ...
    appendIfAvailable(
      point.positions,
      interpolateJointValues(left, left_time, &trajectory_msgs::msg::JointTrajectoryPoint::positions));
    appendIfAvailable(
      point.positions,
      interpolateJointValues(right, right_time, &trajectory_msgs::msg::JointTrajectoryPoint::positions));

    if (merge_velocities) {
      appendIfAvailable(
        point.velocities,
        interpolateJointValues(left, left_time, &trajectory_msgs::msg::JointTrajectoryPoint::velocities));
      appendIfAvailable(
        point.velocities,
        interpolateJointValues(right, right_time, &trajectory_msgs::msg::JointTrajectoryPoint::velocities));
    }
    if (merge_accelerations) {
      appendIfAvailable(
        point.accelerations,
        interpolateJointValues(left, left_time, &trajectory_msgs::msg::JointTrajectoryPoint::accelerations));
      appendIfAvailable(
        point.accelerations,
        interpolateJointValues(right, right_time, &trajectory_msgs::msg::JointTrajectoryPoint::accelerations));
    }
  ...
  const moveit::planning_interface::MoveGroupInterface::Plan & right_plan)
{
  ...
  if (!mergeJointTrajectories(
      left_plan.trajectory.joint_trajectory,
      right_plan.trajectory.joint_trajectory,
      label,
  ...
  moveit::planning_interface::MoveGroupInterface::Plan & plan)
{
  ...
  if (joints.size() != joint_names.size()) {
    RCLCPP_ERROR(
      node->get_logger(),
      "%s has %zu joint values, but group '%s' expects %zu",
      label.c_str(),
      joints.size(),
      arm.getName().c_str(),
      joint_names.size());
...
bool computeSingleArmCartesian(
  const rclcpp::Node::SharedPtr & node,
  moveit::planning_interface::MoveGroupInterface & arm,
  ...
  const std::string & label)
{
  ...
  if (final_point.positions.size() != trajectory.joint_names.size()) {
    RCLCPP_WARN(
      node->get_logger(),
      "%s final point has %zu positions for %zu joints; skipping FK check",
      label.c_str(),
  ...
  RCLCPP_INFO(
    node->get_logger(),
    "%s planned final TCP [%.3f, %.3f, %.3f], target [%.3f, %.3f, %.3f], error [%.3f, %.3f, %.3f]",
    label.c_str(),
    planned_pose.position.x,
    planned_pose.position.y,
    planned_pose.position.z,
    target_pose.position.x,
    target_pose.position.y,
    target_pose.position.z,
    planned_pose.position.x - target_pose.position.x,
    planned_pose.position.y - target_pose.position.y,
    planned_pose.position.z - target_pose.position.z);
}

bool computeSingleArmGraspApproach(
  const rclcpp::Node::SharedPtr & node,
  moveit::planning_interface::MoveGroupInterface & arm,
  const TaskParams & params,
  ...
  const std::string & label)
{
  ...
  auto left_future = std::async(std::launch::async, [&]() {
    return execute_one(left_gripper, label + " left gripper");
  });
  auto right_future = std::async(std::launch::async, [&]() {
    return execute_one(right_gripper, label + " right gripper");
  });
...
bool executeSingleGripperTarget(
  const rclcpp::Node::SharedPtr & node,
  moveit::planning_interface::MoveGroupInterface & gripper,
  const std::string & target,
  const std::string & label)
...
geometry_msgs::msg::Pose correctedPlacePose(
  const ArmContext & context,
  const TaskParams & params)
{
  geometry_msgs::msg::Pose corrected_place_pose =
...
geometry_msgs::msg::Pose correctedLiftPose(
  const ArmContext & context,
  const TaskParams & params)
{
  geometry_msgs::msg::Pose lifted_cup_pose = context.cup_pose;
...
geometry_msgs::msg::Pose poseWithAxes(
  const geometry_msgs::msg::Pose & pose,
...
geometry_msgs::msg::Pose poseWithZAxisClosestToCurrent(
  const geometry_msgs::msg::Pose & pose,
  Eigen::Vector3d z_axis)
{
  if (z_axis.norm() < 1e-6) {
  ...
  const Eigen::Quaterniond current_orientation(
    ...
    pose.orientation.z);
  Eigen::Vector3d x_axis =
    current_orientation.normalized().toRotationMatrix().col(0);
  x_axis -= x_axis.dot(z_axis) * z_axis;
  if (x_axis.norm() < 1e-6) {
    x_axis = std::abs(z_axis.dot(Eigen::Vector3d::UnitX())) < 0.9 ?
...
std::vector<geometry_msgs::msg::Pose> stirWaypoints(
  const geometry_msgs::msg::Pose & stir_center_pose,
  const TaskParams & params)
{
  std::vector<geometry_msgs::msg::Pose> waypoints;
...
std::vector<double> reversedFlatJointWaypoints(
  const std::vector<double> & flat_joint_waypoints,
  size_t joint_count)
{
  ...
  double max_segment_length)
{
  ...
  const int segment_count = std::max(
    1,
    static_cast<int>(std::ceil(distance / std::max(max_segment_length, 1e-3))));
...
bool executeDualCupStepRange(
  ...
  moveit::planning_interface::MoveGroupInterface & right_gripper,
  const ArmConfig & left_config,
  const ArmConfig & right_config,
  const TaskParams & params,
  ArmContext & left_context,
  ...
  size_t end_index)
{
  ...
  if (should_run(1)) {
    ...
    if (!planSingleArmPose(
        node, left_arm, left_context.pre_grasp_pose, left_config.tcp_link, "left pre-grasp", left_plan) ||
      !planSingleArmPose(
        node, right_arm, right_context.pre_grasp_pose, right_config.tcp_link, "right pre-grasp", right_plan) ||
      !executeMergedPlan(node, both_arms, "dual pre-grasp", left_plan, right_plan))
    {
      return false;
    }
    logPoseDelta(node, left_arm, left_config.tcp_link, left_context.pre_grasp_pose, "left pre-grasp");
    logPoseDelta(node, right_arm, right_config.tcp_link, right_context.pre_grasp_pose, "right pre-grasp");
  }
  ...
  if (should_run(3)) {
    ...
    if (!computeSingleArmGraspApproach(
        node,
        left_arm,
        params,
        left_context.tcp_grasp_pose,
        left_config.tcp_link,
        "left grasp approach",
        left_plan) ||
      !computeSingleArmGraspApproach(
        node,
        right_arm,
        params,
        right_context.tcp_grasp_pose,
        right_config.tcp_link,
        "right grasp approach",
        right_plan) ||
    ...
    logPoseDelta(node, right_arm, right_config.tcp_link, right_context.tcp_grasp_pose, "right grasp");
  }

  if (should_run(4)) {
    RCLCPP_INFO(node->get_logger(), "Cup step [cup_close]: close grippers");
    if (!executeDualGripperTarget(
        node, left_gripper, right_gripper, params.cup_grasp_profile.gripper_target, "dual close"))
    {
      return false;
    }
    rclcpp::sleep_for(std::chrono::milliseconds(500));
  ...
  if (should_run(5)) {
    ...
    if (!computeSingleArmCartesian(
        node, left_arm, params, {left_context.lift_pose}, "left lift", left_plan) ||
      !computeSingleArmCartesian(
        node, right_arm, params, {right_context.lift_pose}, "right lift", right_plan))
    {
      return false;
    }
    logPlannedFinalTcpPose(
      node, left_arm, left_plan, left_config.tcp_link, left_context.lift_pose, "left lift");
    logPlannedFinalTcpPose(
      node, right_arm, right_plan, right_config.tcp_link, right_context.lift_pose, "right lift");
...
bool executeDualPickAndLift(
  ...
  ArmContext & right_context)
{
  return executeDualCupStepRange(
    node,
    both_arms,
    left_arm,
    left_gripper,
...
bool executeDualCupApproachOnly(
  const rclcpp::Node::SharedPtr & node,
  moveit::planning_interface::MoveGroupInterface & both_arms,
  moveit::planning_interface::MoveGroupInterface & left_arm,
  moveit::planning_interface::MoveGroupInterface & left_gripper,
...
bool executeOverlappedPostWaterSequence(
  ...
  moveit::planning_interface::MoveGroupInterface & right_gripper,
  const TaskParams & params,
  const ArmContext & left_context,
  const ArmContext & right_context,
  const geometry_msgs::msg::Pose & mixing_cup_pose)
{
  moveit::planning_interface::MoveGroupInterface::Plan left_plan;
  moveit::planning_interface::MoveGroupInterface::Plan right_plan;
  ...
  RCLCPP_INFO(
    node->get_logger(),
    "Dual-arm post-water phase: right water place lower while left ingredient moves to mixing hover");
  ...
  if (!computeSingleArmCartesian(
      node, right_arm, params, {right_place_pose}, "right water place lower", right_plan) ||
    !computeSingleArmCartesian(
      node,
      left_arm,
      params,
      {left_lift_clearance_pose, left_context.mixing_hover_pose},
      "left ingredient mixing hover",
      left_plan) ||
    !executeMergedPlan(node, both_arms, "dual left ingredient hover/right water place", left_plan, right_plan))
  {
    return false;
  }
  ...
  RCLCPP_INFO(
    node->get_logger(),
    "[left/ingredient] Dynamic pour angle: %.3f rad",
    computePourAngleTowardTarget(
      left_context.mixing_hover_pose,
      mixing_cup_pose,
      left_context.horizontal_lateral_vector,
      params.pour_angle));
  if (!computeSingleArmCartesian(
      node, right_arm, params, {right_context.pre_grasp_pose}, "right water retreat", right_plan) ||
    !computeSingleArmCartesian(
      node,
      left_arm,
      params,
      pourTiltWaypoints(left_context, mixing_cup_pose, params),
      "left ingredient pour tilt",
      left_plan) ||
    !executeMergedPlan(node, both_arms, "dual left ingredient pour/right water retreat", left_plan, right_plan))
  {
    return false;
  }
  ...
  if (!planGroup(node, right_arm, "right water home", right_plan) ||
    !computeSingleArmCartesian(
      ...
      "left ingredient wrist upright",
      left_plan) ||
    !executeMergedPlan(
      node, both_arms, "dual left ingredient upright/right water home", left_plan, right_plan))
  {
    return false;
  }
  ...
  RCLCPP_INFO(
    node->get_logger(),
    "Dual-arm post-water phase: left ingredient clears mixing area");
  ...
  if (!executeSingleGripperTarget(node, right_gripper, params.final_gripper_target, "right water final gripper")) {
    return false;
  }

  RCLCPP_INFO(node->get_logger(), "[left/ingredient] Place phase: lower and release");
  if (!computeSingleArmCartesian(
      node, left_arm, params, {left_place_pose}, "left ingredient place lower", left_plan) ||
  ...
  if (!computeSingleArmCartesian(
      node, left_arm, params, {left_context.pre_grasp_pose}, "left ingredient retreat", left_plan) ||
  ...
  return executeSingleGripperTarget(
    node, left_gripper, params.final_gripper_target, "left ingredient final gripper");
}

bool executeSingleArmCartesian(
  const rclcpp::Node::SharedPtr & node,
  moveit::planning_interface::MoveGroupInterface & arm,
  const TaskParams & params,
  const std::vector<geometry_msgs::msg::Pose> & waypoints,
...
bool executePickupPlacePrimitive(
  ...
  ArmTask & right_task,
  const TaskParams & params,
  ArmContext & mixing_context,
  const geometry_msgs::msg::Pose & staged_cup_pose,
  ...
  const std::optional<geometry_msgs::msg::Pose> & prepared_current_high_pose)
{
  ...
  RCLCPP_INFO(
    node->get_logger(),
    "Pickup place: move mixing cup to pickup zone [%.3f, %.3f, %.3f]",
    pickup_zone_pose.position.x,
    pickup_zone_pose.position.y,
    pickup_zone_pose.position.z);

  const Eigen::Isometry3d pickup_zone_transform = poseToIsometry(pickup_zone_pose);
  const Eigen::Vector3d pickup_transfer_point(
    params.pickup_transfer_x,
    params.pickup_transfer_y,
    pickup_zone_pose.position.z);
  Eigen::Vector3d local_pickup_point =
    pickup_zone_transform.inverse() * pickup_transfer_point;
  ...
  RCLCPP_INFO(
    ...
    pickup_cup_pose.position.x,
    pickup_cup_pose.position.y,
    pickup_cup_pose.position.z);
  ...
  if (!prepared_current_high_pose.has_value()) {
    ...
    if (!right_task.executeCartesianWaypoints(
        {current_high_pose},
  ...
  if (!params.right_pickup_transfer_guide_joints.empty()) {
    ...
    if (!right_task.moveThroughJointValueWaypoints(
    ...
    const auto guided_hover_cup_pose =
      cupPoseForTcpPose(mixing_context, pickup_hover_tcp_pose);
    ...
    guided_place_cup_pose.position.z =
      pickup_zone_pose.position.z + params.pickup_cup_center_height_offset;
    const auto guided_place_tcp_pose =
      tcpPoseForCupPositionWithTcpOrientation(
        mixing_context,
        guided_place_cup_pose,
        pickup_hover_tcp_pose.orientation);
    ...
    RCLCPP_INFO(
      node->get_logger(),
      "Pickup place: guided place cup target [%.3f, %.3f, %.3f]",
      guided_place_cup_pose.position.x,
      guided_place_cup_pose.position.y,
      guided_place_cup_pose.position.z);
    used_pickup_transfer_guide = true;
  } else {
    const auto computed_pickup_hover_tcp_pose =
      tcpPoseForCupPositionWithTcpOrientation(
        mixing_context,
        pickup_hover_cup_pose,
        current_high_pose.orientation);
    const auto computed_pickup_place_tcp_pose =
      tcpPoseForCupPositionWithTcpOrientation(
        mixing_context,
        pickup_cup_pose,
        current_high_pose.orientation);
    ...
    if (!right_task.moveToPoseWithFallbacks(
        pickup_transfer_y_pose,
        "mixing cup pickup transfer y") ||
      !right_task.moveToPoseWithFallbacks(
        pickup_transfer_pose,
        "mixing cup pickup transfer x") ||
      !right_task.moveToPoseWithFallbacks(
        computed_pickup_hover_tcp_pose.value(),
  ...
  if (used_pickup_transfer_guide && !params.right_pickup_home_guide_joints.empty()) {
    ...
    return right_task.moveHomeViaGuideAndClose(
      "right pickup place home",
...
bool executeStirAndPickupStage(
  const rclcpp::Node::SharedPtr & node,
  moveit::planning_interface::MoveGroupInterface & both_arms,
  moveit::planning_interface::MoveGroupInterface & left_arm,
  moveit::planning_interface::MoveGroupInterface & left_gripper,
  ...
  bool run_grasp,
  bool run_pick_lift,
  bool stop_after_stick_close,
  bool stop_after_stick_hover,
  bool run_motion,
  bool run_cleanup,
  bool run_pickup_place)
{
  ...
  RCLCPP_INFO(
    ...
    mixing_context->tcp_grasp_pose.position.x,
    mixing_context->tcp_grasp_pose.position.y,
    mixing_context->tcp_grasp_pose.position.z,
    mixing_context->gripper_center_forward_offset);
  if (params.right_mixing_pregrasp_lateral_offset > 0.0) {
    ...
    RCLCPP_INFO(
      node->get_logger(),
      "Stir stage: adjusted right mixing pre-grasp to [%.3f, %.3f, %.3f]",
      mixing_context->pre_grasp_pose.position.x,
      mixing_context->pre_grasp_pose.position.y,
  ...
  if (params.stir_stick_grasp_profile.point_mode == GraspPointMode::HorizontalStickEnd) {
    rotateContextLocal(stick_context.value(), "local_z", params.stir_stick_grasp_profile.stick_roll_angle);
    RCLCPP_INFO(
      node->get_logger(),
  ...
  if (run_pick_lift) {
    ...
    const auto staged_low_tcp_pose =
      tcpPoseForCupPose(mixing_context.value(), staged_cup_pose);
    ...
    auto stage_mixing_cup = [&]() {
      RCLCPP_INFO(
        ...
        staged_cup_pose.position.x,
        staged_cup_pose.position.y,
    ...
    if (run_grasp) {
      ...
      if (has_left_entry_guide) {
          ...
          params.left_stir_entry_guide_joints.size() == left_joint_count)
        {
          RCLCPP_INFO(
            node->get_logger(),
            "Stir stage: move through manual left stir entry guide while right arm moves to mixing pre-grasp");
          if (!planSingleArmJointValues(
              node,
              left_arm,
            ...
            !executeMergedPlan(
              node,
              both_arms,
              "stir left entry guide / right mixing pre-grasp",
              left_plan,
              right_plan))
          {
            return false;
          ...
          if (!left_task.moveThroughJointValueWaypoints(
              params.left_stir_entry_guide_joints,
              "left stir entry guide joints") ||
      ...
      } else if (!executeMergedPlan(
          node,
          both_arms,
          "stir mixing pre-grasp / stick pre-grasp",
          left_plan,
          right_plan))
      {
        return false;
      ...
      if (!executeDualGripperTarget(
          node, left_gripper, right_gripper, "open", "stir prep dual open"))
      {
        return false;
      }
      ...
      if (!planSingleArmPose(
        ...
        !executeMergedPlan(
          node,
          both_arms,
          "stir mixing grasp / stick pre-grasp",
          left_plan,
          right_plan))
      {
        return false;
      ...
      if (!right_task.closeGripperTo(
          params.cup_grasp_profile.gripper_target,
      ...
      if (!planSingleArmPose(
          node,
          left_arm,
          stick_context->tcp_grasp_pose,
          left_config.tcp_link,
          "left stir stick grasp approach",
          left_plan) ||
        !computeSingleArmCartesian(
          node,
          right_arm,
          params,
          {mixing_context->lift_pose},
          "right mixing lift",
          right_plan) ||
        !executeMergedPlan(node, both_arms, "stir mixing lift / stick grasp", left_plan, right_plan))
      {
        return false;
      }

      if (!left_task.closeGripperTo(
      ...
      if (!computeSingleArmCartesian(
          ...
          left_arm,
          params,
          {stick_context->lift_pose},
          "left stir stick lift from holder",
          left_plan) ||
        !computeSingleArmCartesian(
          node,
          right_arm,
          params,
          {staged_high_tcp_pose.value(), staged_low_tcp_pose.value()},
          "right mixing cup center stage and lower",
          right_plan) ||
        !executeMergedPlan(node, both_arms, "stir mixing stage / stick lift", left_plan, right_plan))
      {
        return false;
      }
    } else {
    ...
    stick_hover_pose.position.x =
      staged_cup_pose.position.x -
      stick_context->horizontal_approach_vector.x() * stick_context->gripper_center_forward_offset;
    stick_hover_pose.position.y =
      staged_cup_pose.position.y -
      stick_context->horizontal_approach_vector.y() * stick_context->gripper_center_forward_offset;
    ...
    RCLCPP_INFO(
      ...
      inserted_stick_pose.position.x,
      inserted_stick_pose.position.y,
      inserted_stick_pose.position.z);
  ...
  if (run_motion) {
    RCLCPP_INFO(
      node->get_logger(),
      "Stir stage: Cartesian circular stir around current inserted TCP center [%.3f, %.3f, %.3f], radius %.3f, cycles %d",
      stir_center_pose.position.x,
      stir_center_pose.position.y,
      stir_center_pose.position.z,
      params.stir_radius,
      params.stir_cycles);
  ...
  if (run_cleanup) {
    ...
    if (!left_task.executeCartesianWaypoints(
        {stick_context->tcp_grasp_pose},
    ...
    if (run_pickup_place) {
      ...
      if (!computeSingleArmCartesian(
        ...
        !computeSingleArmCartesian(
          ...
          right_arm,
          params,
          {pickup_initial_high_pose},
          "right mixing cup initial pickup raise",
          right_raise_plan) ||
        !executeMergedPlan(
          node,
          both_arms,
          "stir holder backout / pickup initial raise",
          left_backout_plan,
          right_raise_plan))
      {
        return false;
  ...
  if (run_pickup_place) {
    if (left_cleanup_home_deferred) {
      ...
      if (!planGroup(node, left_arm, "left stir cleanup home", left_home_plan) ||
        ...
        !executeSingleGripperTarget(
          node, left_gripper, params.final_gripper_target, "left stir cleanup final gripper"))
      {
        return false;
      }
      if (!executePickupPlacePrimitive(
          node,
          right_task,
          params,
          mixing_context.value(),
          staged_cup_pose,
          pickup_zone_pose,
    ...
    } else if (!executePickupPlacePrimitive(
        node,
        right_task,
        params,
        mixing_context.value(),
  ...
  const geometry_msgs::msg::Pose & mixing_cup_pose)
{
  ...
  context.lift_pose =
    tcpPoseForCupPose(context, desired_lift_cup_pose).value_or(tcp_pose);
  if (!context.tcp_to_cup_transform.has_value()) {
  ...
  const Eigen::Vector3d tcp_to_cup_translation =
    context.tcp_to_cup_transform->translation();
  RCLCPP_INFO(
    node->get_logger(),
    "[%s] Refreshed held cup state: cup [%.3f, %.3f, %.3f], tcp [%.3f, %.3f, %.3f], tcp->cup [%.3f, %.3f, %.3f], lift tcp [%.3f, %.3f, %.3f]",
    config.label.c_str(),
    actual_cup_pose.position.x,
    actual_cup_pose.position.y,
    actual_cup_pose.position.z,
    tcp_pose.position.x,
    tcp_pose.position.y,
    tcp_pose.position.z,
    tcp_to_cup_translation.x(),
    tcp_to_cup_translation.y(),
    tcp_to_cup_translation.z(),
    context.lift_pose.position.x,
    context.lift_pose.position.y,
    context.lift_pose.position.z);
}
...
std::optional<size_t> findStepIndex(
...
bool rangeIncludes(
  size_t start_step_index,
  size_t end_step_index,
...
void refreshHeldCupTargetsFromGazebo(
  const rclcpp::Node::SharedPtr & node,
  ...
  std::optional<ArmContext> & right_context)
{
  ...
  if (cup_start <= kCupCloseIndex && cup_end >= kCupLiftIndex) {
    cup_ok = executeDualCupStepRange(
      ...
      kCupCloseIndex);
    if (cup_ok) {
      refreshHeldCupTargetsFromGazebo(
        node,
        pose_mutex,
        pose_cache,
        left_arm,
  ...
  std::optional<ArmContext> & right_context)
{
  ...
  const PoseCache current_poses = latestPoseCache(pose_mutex, pose_cache);
  const auto water_clear_mixing_pose =
    current_poses.mixing.value_or(initial_poses.mixing.value());
  return right_context.has_value() &&
    right_task.clearAfterPour(right_context.value(), water_clear_mixing_pose);
}

bool runIngredientPourPlacePrimitive(
  const rclcpp::Node::SharedPtr & node,
  moveit::planning_interface::MoveGroupInterface & both_arms,
  moveit::planning_interface::MoveGroupInterface & left_arm,
  moveit::planning_interface::MoveGroupInterface & left_gripper,
  ...
  std::optional<ArmContext> & right_context)
{
  ...
  const PoseCache current_poses = latestPoseCache(pose_mutex, pose_cache);
  const auto ingredient_mixing_pose =
    current_poses.mixing.value_or(initial_poses.mixing.value());
  if (!left_context.has_value() || !right_context.has_value()) {
    return false;
  }
  return executeOverlappedPostWaterSequence(
    node,
    both_arms,
    left_arm,
    left_gripper,
    ...
    left_context.value(),
    right_context.value(),
    ingredient_mixing_pose);
}

bool runPreStirBoundaryPrimitive(
  const rclcpp::Node::SharedPtr & node,
  moveit::planning_interface::MoveGroupInterface & both_arms,
  moveit::planning_interface::MoveGroupInterface & left_arm,
  moveit::planning_interface::MoveGroupInterface & left_gripper,
  ...
  const TaskParams & params)
{
  ...
  if (!planGroup(node, left_arm, "left pre-stir boundary home", left_home_plan) ||
    ...
    !executeMergedPlan(
      node, both_arms, "pre-stir boundary merged home", left_home_plan, right_home_plan) ||
    !executeDualGripperTarget(
      node, left_gripper, right_gripper, params.final_gripper_target, "pre-stir boundary close"))
  {
...
bool runStirAndPickupPrimitiveRange(
  const rclcpp::Node::SharedPtr & node,
  moveit::planning_interface::MoveGroupInterface & both_arms,
  moveit::planning_interface::MoveGroupInterface & left_arm,
  moveit::planning_interface::MoveGroupInterface & left_gripper,
  ...
  const std::string & effective_end_step)
{
  ...
  RCLCPP_INFO(
    ...
    stir_poses.mixing->position.x,
    stir_poses.mixing->position.y,
    stir_poses.mixing->position.z,
    stir_poses.stir_stick->position.x,
    stir_poses.stir_stick->position.y,
    stir_poses.stir_stick->position.z);
  ...
  return executeStirAndPickupStage(
    ...
    right_task,
    params,
    stir_poses.mixing.value(),
    stir_poses.stir_stick.value(),
...
int main(int argc, char ** argv)
{
  ...
  const double planning_time =
    getOrDeclareParameter<double>(node, "planning_time", 5.0);
  const int planning_attempts =
    getOrDeclareParameter<int>(node, "planning_attempts", 5);
  const std::string start_step =
    getOrDeclareParameter<std::string>(node, "start_step", "");
  const std::string end_step =
    getOrDeclareParameter<std::string>(node, "end_step", "");
  const bool requested_run_water =
    getOrDeclareParameter<bool>(node, "run_water", true);
  const bool requested_run_ingredient =
    getOrDeclareParameter<bool>(node, "run_ingredient", true);
  ...
  const std::string effective_start_step = start_step.empty() ? task_step_names.front() : start_step;
  ...
  if (!start_step_index.has_value() || !end_step_index.has_value()) {
    RCLCPP_ERROR(
      node->get_logger(),
      "Invalid step range '%s' -> '%s'. Supported steps: cup_ready, cup_pre_grasp, cup_open, cup_grasp, cup_close, cup_lift, water_pour, water_clear, ingredient_pour_place, stir_grasp, stir_pick_lift, stir_stick_close, stir_hover, stir_motion, stir_cleanup, pickup_place",
      effective_start_step.c_str(),
      effective_end_step.c_str());
    rclcpp::shutdown();
    spinner.join();
    return 1;
  }
  ...
  const bool requires_cup_tasks =
    rangeIncludes(start_step_index.value(), end_step_index.value(), kCupReadyIndex, kIngredientPourPlaceIndex);
  const bool requires_stir_tasks =
    rangeIncludes(start_step_index.value(), end_step_index.value(), kStirGraspIndex, kPickupPlaceIndex);
  const bool requires_pickup_zone_task =
    rangeIncludes(start_step_index.value(), end_step_index.value(), kPickupPlaceIndex, kPickupPlaceIndex);
  const bool requires_mixing_pose = requires_cup_tasks || requires_stir_tasks;
  const bool run_water =
    requires_cup_tasks && requested_run_water;
  const bool run_ingredient =
    requires_cup_tasks && requested_run_ingredient;
  ...
  const bool vertical_stir_stick =
    getOrDeclareParameter<bool>(node, "vertical_stir_stick", true);
  ...
  params.stir_cup_stage_y =
    getOrDeclareParameter<double>(node, "stir_cup_stage_y", 0.0);
  ...
  params.pickup_transfer_x =
    getOrDeclareParameter<double>(node, "pickup_transfer_x", 0.25);
  params.pickup_transfer_y =
    getOrDeclareParameter<double>(node, "pickup_transfer_y", -0.425);
  params.pickup_table_size_x =
    getOrDeclareParameter<double>(node, "pickup_table_size_x", 0.65);
  params.pickup_table_size_y =
    getOrDeclareParameter<double>(node, "pickup_table_size_y", 0.27);
  ...
  params.dynamic_waypoint_lift =
    getOrDeclareParameter<double>(node, "dynamic_waypoint_lift", 0.08);
  params.right_pickup_transfer_guide_joints =
  ...
  RCLCPP_INFO(
    ...
    left_tcp_set ? "ok" : "failed",
    right_arm.getEndEffectorLink().c_str(),
    right_tcp_set ? "ok" : "failed");
  ...
  const bool subscribed = gz_node.Subscribe<gz::msgs::Pose_V>(
    gazebo_pose_topic,
  ...
  if (!subscribed) {
    RCLCPP_ERROR(node->get_logger(), "Failed to subscribe to Gazebo pose topic: %s", gazebo_pose_topic.c_str());
    rclcpp::shutdown();
    spinner.join();
    return 1;
  }
  ...
  RCLCPP_INFO(
    node->get_logger(),
    "Step range '%s' -> '%s': waiting for water '%s', ingredient '%s', mixing '%s', stir stick '%s', pickup '%s' poses on %s",
    effective_start_step.c_str(),
    effective_end_step.c_str(),
    water_model.c_str(),
  ...
  {
    {
      ...
      const bool have_stir_stick =
        !requires_stir_tasks || pose_cache.stir_stick.has_value();
      const bool have_pickup =
        !requires_pickup_zone_task || pose_cache.pickup.has_value();
      const bool have_mixing =
        !requires_mixing_pose || pose_cache.mixing.has_value();
      if (have_water && have_ingredient && have_mixing && have_stir_stick &&
        have_pickup)
      {
        break;
      }
  ...
  if (run_ingredient && !poses.ingredient.has_value()) {
    RCLCPP_ERROR(
      node->get_logger(), "Timed out waiting for ingredient model '%s'", ingredient_model.c_str());
    rclcpp::shutdown();
    spinner.join();
    return 1;
  }
  if (requires_mixing_pose && !poses.mixing.has_value()) {
    RCLCPP_ERROR(node->get_logger(), "Timed out waiting for mixing model '%s'", mixing_model.c_str());
    rclcpp::shutdown();
    spinner.join();
    return 1;
  }
  if (requires_stir_tasks && !poses.stir_stick.has_value()) {
    RCLCPP_ERROR(
      node->get_logger(), "Timed out waiting for stir stick model '%s'", stir_stick_model.c_str());
    rclcpp::shutdown();
    spinner.join();
    return 1;
  }
  if (requires_pickup_zone_task && !poses.pickup.has_value()) {
    RCLCPP_ERROR(
      node->get_logger(), "Timed out waiting for pickup model '%s'", pickup_model.c_str());
    rclcpp::shutdown();
    spinner.join();
    return 1;
  }
  ...
  RCLCPP_INFO(
    node->get_logger(),
    "Executing primitive step range '%s' -> '%s'",
    effective_start_step.c_str(),
    effective_end_step.c_str());

  if (!runCupPrimitiveSteps(
      node,
      both_arms,
      left_arm,
      left_gripper,
      ...
      run_water,
      run_ingredient,
      start_step_index.value(),
      end_step_index.value(),
      pose_mutex,
      pose_cache,
      left_context,
      right_context))
  ...
  if (stepInRange(kIngredientPourPlaceIndex, start_step_index.value(), end_step_index.value())) {
    refreshHeldCupTargetsFromGazebo(
      ...
      left_context,
      right_context);
    if (!runIngredientPourPlacePrimitive(
        node,
        both_arms,
        left_arm,
        left_gripper,
  ...
  if (requires_cup_tasks &&
    rangeIncludes(start_step_index.value(), end_step_index.value(), kStirGraspIndex, kPickupPlaceIndex))
  {
    if (!runPreStirBoundaryPrimitive(
        node, both_arms, left_arm, left_gripper, right_arm, right_gripper, params))
    {
      rclcpp::shutdown();
      spinner.join();
      return 1;
