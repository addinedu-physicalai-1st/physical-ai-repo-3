...
bool OpenArmHW::parse_config(const hardware_interface::HardwareInfo& info) {
  auto parse_bool = [](std::string value, bool default_value) {
  ...
  if (it == info.hardware_parameters.end()) {
    ...
    hand_ = parse_bool(it->second, true);
  }
  ...
  if (it == info.hardware_parameters.end()) {
    ...
    return_to_zero_on_activate_ = parse_bool(it->second, false);
  }

  it = info.hardware_parameters.find("hold_current_on_activate");
  if (it == info.hardware_parameters.end()) {
    hold_current_on_activate_ = true;
  } else {
    hold_current_on_activate_ = parse_bool(it->second, true);
  }
...
OpenArmHW::export_state_interfaces() {
  ...
  const size_t arm_joint_count = std::min(ARM_DOF, joint_names_.size());
  ...
  if (hand_ && joint_names_.size() > ARM_DOF) {
    state_interfaces.emplace_back(hardware_interface::StateInterface(
        joint_names_[ARM_DOF], hardware_interface::HW_IF_POSITION,
        &pos_states_[ARM_DOF]));
    state_interfaces.emplace_back(hardware_interface::StateInterface(
        joint_names_[ARM_DOF], hardware_interface::HW_IF_VELOCITY,
        &vel_states_[ARM_DOF]));
  }
...
hardware_interface::CallbackReturn OpenArmHW::on_activate(
  ...
  if (hold_current_on_activate_) {
    sync_current_state_to_commands();
  }

  if (return_to_zero_on_activate_) {
    return_to_zero();
  } else {
    RCLCPP_INFO(
        rclcpp::get_logger("OpenArmHW"),
        "Skipping zero return on activation; holding current command state");
  }
...
void OpenArmHW::sync_current_state_to_commands() {
  openarm_->refresh_all();
  openarm_->recv_all();
  ...
  RCLCPP_INFO(
      rclcpp::get_logger("OpenArmHW"),
      "Initialized command state from current hardware position");
}
