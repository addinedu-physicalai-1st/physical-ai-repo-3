...
inline constexpr MotionScalingPreset kDefaultMotionScaling{0.05, 0.05, 0.12, 0.12};

inline constexpr PlanningPreset kDefaultPlanning{8.0, 8, 3.5};

inline constexpr PickGeometryPreset kDefaultPickGeometry{0.12, 0.12, 0.10, 0.10};

inline constexpr PlaceGeometryPreset kDefaultPlaceGeometry{0.08, 0.005, 0.000};

inline constexpr CartesianPreset kDefaultCartesian{0.005, 0.90, false, 3.5};

inline constexpr PlanningScenePreset kDefaultPlanningScene{true, 300};

inline constexpr KetchupSqueezePreset kDefaultKetchupSqueeze{0.12, 0.060, 0.010};
...
// 예: Pick 내부에서 pre_grasp만 활성화하면 pre-grasp 자세만 고정하고 grasp/close/lift는 계속 자동
// 진행한다. waypoint name과 개수는 target/task마다 다르게 정의할 수 있다. 왼손 pose 확인:
// ROS_LOG_DIR=/tmp/ros_logs ros2 run tf2_ros tf2_echo world openarm_left_hand_tcp 오른손 pose 확인:
// ROS_LOG_DIR=/tmp/ros_logs ros2 run tf2_ros tf2_echo world openarm_right_hand_tcp
...
// 빵 pick 파지 세부 조정값.
inline constexpr PickTuningPreset kLeftBreadPickTuning{0.0, {true, 0.030}, {true, 0.021}};

// 케이스 pick 파지 세부 조정값.
inline constexpr PickTuningPreset kRightCasePickTuning{-0.035, {true, 0.043}, {true, 0.034}};
...
// 케이스 grasp 시 면에 너무 붙어 MoveIt gripper close collision이 나는 것을 피하기 위한 world z
// 여유. 깊이는 kRightCasePickTuning.grasp_tcp_z_offset_m으로 유지하고 높이만 올린다.
inline constexpr double kRightCaseGraspWorldZOffsetM = 0.004;

// 소시지 pick 파지 세부 조정값.
inline constexpr PickTuningPreset kLeftSausagePickTuning{0.000, {true, 0.030}, {true, 0.008}};

// 케첩 pick 파지 세부 조정값.
inline constexpr PickTuningPreset kLeftKetchupPickTuning{0.010, {true, 0.040}, {true, 0.014}};
...
inline constexpr PickTuningPreset kLeftBeverageCanPickTuning{0.0, {true, 0.043}, {true, 0.018}};

inline constexpr PickTuningPreset kRightBeverageCanReceiveTuning{0.0, {true, 0.043}, {true, 0.018}};

// 음료 캔은 왼손이 캔 중심보다 위를 잡고, 오른손이 캔 중심보다 아래를 잡는다.
// horizontal pick에서 grasp_tcp_z_offset_m은 world z가 아니므로, 음료 전용 world z offset을 별도로
// 둔다.
...
inline const StageWaypointPosePreset* findStageWaypointPosePreset(ManufacturingTarget target,
                                                                  ArmSide arm,
                                                                  ManufacturingStage stage,
                                                                  const char* name) {
