...
class DrinkServingNode : public rclcpp::Node {
 public:
  DrinkServingNode() : Node("ddooby_drink_serving_node") {
  ...
  bool runScenario() {
    RCLCPP_INFO(get_logger(),
                ...
                item_name_.c_str(), drink_model_.c_str(), scenario_only_ ? "true" : "false");

    const std::vector<ScenarioStep> steps{
        {"fridge_open", "right hand opens the refrigerator door"},
        {"drink_pick",
         "right hand picks the requested drink from the configured refrigerator slot"},
    ...
    for (size_t i = 0; i < steps.size(); ++i) {
      ...
      RCLCPP_INFO(get_logger(), "Drink scenario step %zu/%zu [%s]: %s", i + 1, steps.size(),
