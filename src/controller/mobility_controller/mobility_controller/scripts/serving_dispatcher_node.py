...
class ServingDispatcher(Node):
    ...
    def _start_next_nav(self) -> None:
        ...
        if self._send_nav_goal(entry['pose'], context=f'table {cmd}'):
            self.state = State.NAVIGATING
        else:
            self.get_logger().warn(
                f'테이블 {cmd} nav 시작 실패 — drop, 다음 액션')
            self.current_table = None
            self._after_dwell()
    ...
    def _send_through_poses(self, poses: list[PoseStamped], context: str) -> bool:
            ...
            if success:
                if self.route_final_n2p and self._route_final_pose is not None:
                    ...
                    if self._send_nav_goal(
                            self._route_final_pose,
                            context=f'{self.current_table} final'):
                        self.state = State.FINALIZING
                    else:
                        self.get_logger().warn(
                            f'경로 {self.current_table} 마지막 xy 정렬 시작 실패 — '
                            'yaw 정렬 생략')
                        self._finish_route()
                else:
                    self.get_logger().info(
    ...
    def _after_recovery(self) -> None:
        ...
        if self._send_through_poses(
                list(poses),
                context=f'{route_id}:{leg} (retry {self._route_retries})'):
            self.state = State.ROUTING
        else:
            self.state = State.IDLE
            self.current_table = None
