...
from scout_follow_controller_node import (  # noqa: E402
    cx_to_angular, bh_to_linear, dist_to_linear, front_min_range,
    yaw_from_quat, handoff_servo_pan, handoff_done,
...
# --- dist_to_linear (라이다 전방거리 → 전진, 후진 금지) ---

def test_dist_far_goes_forward():
    assert dist_to_linear(3.0, target_dist=1.5, kp=0.6, max_v=0.22, deadband=0.1) > 0.0


def test_dist_at_target_returns_zero():
    assert dist_to_linear(1.5, target_dist=1.5, kp=0.6, max_v=0.22, deadband=0.1) == 0.0


def test_dist_within_deadband_returns_zero():
    assert dist_to_linear(1.55, target_dist=1.5, kp=0.6, max_v=0.22, deadband=0.1) == 0.0


def test_dist_closer_than_target_never_reverses():
    assert dist_to_linear(0.8, target_dist=1.5, kp=0.6, max_v=0.22, deadband=0.1) == 0.0


def test_dist_none_returns_zero():
    assert dist_to_linear(None, target_dist=1.5, kp=0.6, max_v=0.22, deadband=0.1) == 0.0


def test_dist_zero_or_negative_returns_zero():
    assert dist_to_linear(0.0, target_dist=1.5, kp=0.6, max_v=0.22, deadband=0.1) == 0.0


def test_dist_clamps_to_max_v():
    assert dist_to_linear(10.0, target_dist=1.5, kp=0.6, max_v=0.22, deadband=0.1) == 0.22


# --- front_min_range (정면 섹터 최근접, 장착 오프셋·랩어라운드) ---

def _scan(ranges, angle_min=-math.pi, angle_inc=math.pi / 4):
    return ranges, angle_min, angle_inc


def test_front_min_picks_nearest_in_forward_sector():
    # angle_min=-pi, inc=pi/4 → idx0=-pi ... idx4=0(정면) ... idx8=+pi
    ranges = [9.0, 9.0, 9.0, 9.0, 2.0, 9.0, 9.0, 9.0, 9.0]
    r, amin, inc = _scan(ranges)
    assert front_min_range(r, amin, inc, forward_rad=0.0,
                           half_rad=math.radians(30), rmin=0.25, rmax=5.0) == 2.0


def test_front_min_forward_offset_180():
    # 정면이 라이다 180° (장착 회전) — idx0/idx8 = ±pi
    ranges = [2.0, 9.0, 9.0, 9.0, 9.0, 9.0, 9.0, 9.0, 2.0]
    r, amin, inc = _scan(ranges)
    assert front_min_range(r, amin, inc, forward_rad=math.pi,
                           half_rad=math.radians(30), rmin=0.25, rmax=5.0) == 2.0


def test_front_min_ignores_out_of_sector():
    # 정면 0°: 측면(±90°=idx2,6) 의 가까운 값은 무시
    ranges = [9.0, 9.0, 0.5, 9.0, 3.0, 9.0, 0.5, 9.0, 9.0]
    r, amin, inc = _scan(ranges)
    assert front_min_range(r, amin, inc, forward_rad=0.0,
                           half_rad=math.radians(30), rmin=0.25, rmax=5.0) == 3.0


def test_front_min_filters_inf_nan_and_range():
    ranges = [0.0] * 9
    ranges[4] = float('inf')
    r, amin, inc = _scan(ranges)
    assert front_min_range(r, amin, inc, forward_rad=0.0,
                           half_rad=math.radians(30), rmin=0.25, rmax=5.0) is None


def test_front_min_empty_returns_none():
    assert front_min_range([], -math.pi, math.pi / 4, 0.0,
                           math.radians(30), 0.25, 5.0) is None
