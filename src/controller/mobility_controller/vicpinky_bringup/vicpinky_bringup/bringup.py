...
BATTERY_PUB_TOPIC_NAME = "battery_state"
ODOM_FRAME_ID = "odom"
...
class VicPinky(Node):
    def __init__(self):
        ...
        # --- Battery (ZLAC bus voltage) parameters ---
        # 7S Li-ion 기본 가정. 실 배터리 chemistry 확정 시 조정.
        self.declare_parameter('battery_min_voltage', 21.0)   # ~3.0V/cell
        self.declare_parameter('battery_max_voltage', 29.4)   # 4.2V/cell
        self.declare_parameter('battery_publish_period', 1.0) # sec, 1Hz
        self.battery_min_v = self.get_parameter('battery_min_voltage').value
        self.battery_max_v = self.get_parameter('battery_max_voltage').value
        ...
        # 2026-05-13: RPi 재부팅 후 ZLAC이 enable 명령을 1회 거부하는 사례 발견.
        # 구형 bringup 프로세스가 /dev/motor 를 점유 중이거나 드라이버 fault 상태일 때
        # disable → sleep → enable 재시도로 복구.
        enable_ok = False
        for en_attempt in range(5):
            if en_attempt > 0:
                self.get_logger().warn(f"Enable attempt {en_attempt}/4 failed, disable→sleep→retry...")
                self.driver.disable()
                time.sleep(0.3)
            ...
            time.sleep(0.2)
        if not enable_ok:
            self.get_logger().error("Failed to enable motors after retries! Shutting down.")
            self.driver.terminate()
            return
        ...
        # 2026-05-13: step5 직후 인코더 읽기가 단 1회 실패로 즉시 종료되던 문제 수정.
        # set_double_rpm → get_position 사이 최소 300ms 확보 + 5회 재시도.
        time.sleep(0.3)
        enc_ok = False
        for enc_attempt in range(5):
            self.last_encoder_l, self.last_encoder_r = self.driver.get_position()
            if self.last_encoder_l is not None and self.last_encoder_r is not None:
                enc_ok = True
                break
            self.get_logger().warn(f"Encoder read attempt {enc_attempt + 1}/5 failed, retrying...")
            time.sleep(0.2)
        if not enc_ok:
        ...
        self.battery_pub = self.create_publisher(BatteryState, BATTERY_PUB_TOPIC_NAME, 10)
        ...
        self.battery_timer = self.create_timer(
            self.get_parameter('battery_publish_period').value,
    ...
    def publish_battery_state(self):
        ...
        if v is None:
            ...
            v_max = self.battery_max_v
            if v_max > v_min:
                pct = (v - v_min) / (v_max - v_min)
