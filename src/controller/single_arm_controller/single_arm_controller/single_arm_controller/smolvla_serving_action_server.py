"""
SmolVLA serving ROS action server.

act_serving_action_server.py와 동일한 통신 레이어 구조를 유지하되
SmolVLAServingOrchestrator를 사용한다.
"""
...
from single_arm_controller.smolvla_serving_orchestrator import SmolVLAServingOrchestrator
...
class SmolVLAServingActionServer(Node):
    def __init__(self):
        super().__init__('smolvla_serving_controller')
        ...
        self._orchestrator = SmolVLAServingOrchestrator(
        ...
        self.get_logger().info('SmolVLA serving action server ready (Pickup + Serve)')
    ...
    def _wait_for_palm_direction(self) -> str:
        ...
        if timed_out:
            self.get_logger().warn('palm_direction timeout — defaulting to "left" (blue marker).')
        else:
...
def main(args=None):
    rp.init(args=args)
    node = SmolVLAServingActionServer()
