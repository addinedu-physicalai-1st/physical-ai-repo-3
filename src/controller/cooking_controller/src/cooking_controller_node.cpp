...
public:
  ...
  : Node("cooking_controller"), request_id_(0), running_(true), server_fd_(-1)
  {
    ...
    health_service_ = this->create_service<std_srvs::srv::Trigger>(
      "/cooking_controller/health_check",
      std::bind(
        &CookingControllerNode::handle_health_check,
        this,
        std::placeholders::_1,
        std::placeholders::_2));
    ...
    RCLCPP_INFO(
      this->get_logger(),
      "bridging cooking_controller status to control_service TCP %s:%d",
  ...
  ~CookingControllerNode() override
  {
    running_ = false;
    if (server_fd_ >= 0) {
...
private:
    ...
    std::shared_ptr<std_srvs::srv::Trigger::Response> response)
  {
    ...
    response->message = "cooking_controller health status published";
  }
  ...
  void run_tcp_server()
  {
    ...
    if (server_fd_ < 0) {
      RCLCPP_WARN(this->get_logger(), "failed to create cooking_controller TCP bridge socket");
      return;
    }
    ...
    if (bind(server_fd_, reinterpret_cast<sockaddr *>(&address), sizeof(address)) < 0) {
      RCLCPP_WARN(
        this->get_logger(),
        "failed to bind cooking_controller TCP bridge %s:%d",
        service_listener_host_.c_str(),
        service_listener_port_);
      return;
    }
    ...
    if (listen(server_fd_, 8) < 0) {
      RCLCPP_WARN(this->get_logger(), "failed to listen on cooking_controller TCP bridge");
      return;
    }
