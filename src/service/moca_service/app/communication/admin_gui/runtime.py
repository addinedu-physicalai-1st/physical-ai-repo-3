...
class AdminGuiCommunicationRuntime:
    ...
    def publish_payload(self, topic: int, event: int, payload: bytes = b"") -> bool:
        ...
        if len(frames) > 1:
            with self._lock:
                self._sequence = (self._sequence + len(frames) - 1) & 0xFFFF
        sent = True
        for frame in frames:
            sent = self.publisher.publish_frame(frame) and sent
        return sent
